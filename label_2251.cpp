...
void label_2251()
{
    ...
    if (cdata(150, tc) == 14) {
        ...
        if (stat != 0) {
            {
                ...
                for (int cnt_end = cnt + (stat); cnt < cnt_end; ++cnt) {
                    ...
                    listn(0, listmax) =
                        lang(cdatan(0, rtval(cnt)) + u8"の居場所を聞く"s,
        ...
        if (itemfind(0, 284) != -1) {
            ...
            if (itemfind(0, 283) != -1) {
                list(0, listmax) = 32;
                listn(0, listmax) = lang(u8"落し物のカバンを届ける。"s,
    ...
    if (cdata(257, tc) != 0 || 0) {
        if (gdata(20) != 35) {
            if (tc >= 16) {
                if (evid() == -1) {
                    list(0, listmax) = 56;
                    listn(0, listmax) = lang(u8"気持ちいいことしない？"s,
    ...
    if (f == 1) {
        {
            int cnt = 0;
            for (int cnt_end = cnt + (gdata(75)); cnt < cnt_end; ++cnt) {
                if (qdata(3, cnt) == 0) { continue; }
                if (qdata(8, cnt) != 1) { continue; }
                if (qdata(14, cnt) == 2) {
                    if (qdata(10, cnt) == rq) {
                        inv_getheader(0);
                        p = qdata(11, cnt);
    ...
    if (chatval == 13) {
        ...
        if (cdata(22, 0) >= 15000) {
            buff = lang(u8"腹が減っているようにはみえない"s + _yo(),
    ...
    if (chatval >= 14 && chatval < 17) {
        ...
        if (p == 0) {
            buff = lang(u8"鑑定するアイテムはないみたい"s + _da(),
    ...
    if (chatval == 21 || chatval == 22) {
        ...
        gdata(74) = calcfame(0,
            (220 - gdata(120) / 50) *
                    (100 + limit(adata(22, gdata(20)), 0, 50)) / 100 +
                2);
        listmax = 0;
        ...
        if (chatval == 21) {
            if (adata(26, gdata(20)) > gdata(13) + gdata(12) * 24 +
                ...
                buff = lang(u8"残念だが、今日の試合はもう終了し"s + _ta(),
            ...
            exrand_randomize(adata(24, gdata(20)));
            {
                ...
                for (int cnt_end = cnt + (50); cnt < cnt_end; ++cnt) {
                    ...
                    arenaop(2) = 3;
                    if (gdata(120) / 100 < 30) { arenaop(2) = 4; }
                    if (gdata(120) / 100 < 10) { arenaop(2) = 5; }
                    minlevel = arenaop(1) / 3 * 2;
                    flt(arenaop(1));
                    fixlv = arenaop(2);
                    characreate(56, 0, -3, 0);
                    if (cmshade) { continue; }
                    if (cdata(38, rc) < minlevel) { continue; }
                    if (cdata(69, rc) != -3) { continue; }
                    break;
                }
            }
            arenaop(1) = cdata(27, rc);
            buff = lang(u8"今日の対戦相手は"s + cdatan(0, rc) + _da() +
                    u8"挑戦する"s + _noka(1),
            ...
            buff = lang(u8"対戦相手はレベル"s + arenaop(1) +
                    u8"以下の相手複数"s + _da() + u8"挑戦する"s + _noka(1),
                u8"Your play is a group of monster around level "s +
                    arenaop(1) + u8". Sounds easy huh?"s);
        }
        list(0, listmax) = 1;
        ...
        label_2257();
        if (chatval != 1) {
            buff = lang(u8"用があるときは声をかけて"s + _kure(),
    ...
    if (chatval == 40 || chatval == 41 || chatval == 49) {
        ...
        gdata(74) = calcfame(0,
            (220 - gdata(121) / 50) *
                    (50 + limit(adata(23, gdata(20)), 0, 50)) / 100 +
                2);
        if (chatval == 49) {
            comctrl = 1;
            label_19432();
        ...
        if (chatval == 40) {
            ...
            buff = lang(u8"一対一の戦いで、対戦相手はレベル"s + arenaop(2) +
    ...
    if (chatval == 25) {
        ...
        rq = deliver;
        --inv(0, deliver(1));
        txt(lang(itemname(deliver(1), 1) + u8"を手渡した。"s,
    ...
    if (chatval == 26) {
        ...
        label_2663();
        --inv(0, supply);
        txt(lang(itemname(supply, 1) + u8"を手渡した。"s,
    ...
    if (chatval == 32) {
        ...
        if (inv(25, p) == 0) {
            ...
            buff = lang(u8"わざわざ落し物を届けてくれた"s + _noka() + _kimi(3) +
            ...
            if (gdata(851) >= 4) {
                listmax = 0;
                buff = lang(u8"む、また"s + _kimi(3) + _ka() +
    ...
    if (chatval == 33) {
        ...
        if (stat != -1) {
            rc = stat;
            if (cdata(0, rc) == 1) {
                buff = lang(u8"そいつは呼び戻す必要はないよう"s + _da(),
                    u8"Huh? You don't need to do that."s);
                label_2251();
                return;
            }
            listmax = 0;
            buff = lang(u8"そいつを呼び戻すには、"s + calcresurrectvalue(rc) +
                    strgold + u8"必要"s + _da(),
            ...
            if (chatval == 1) {
                ...
                buff = lang(u8"(バーテンが店の奥から"s + name(rc) +
                        u8"を連れてきた)"s + _dozo(),
    ...
    if (chatval == 36 || chatval == 57) {
        ...
        label_2257();
        if (chatval == 1) {
            txt(lang(cdatan(0, 56) + u8"を買い取った。"s,
    ...
    if (chatval == 37) {
        ...
        if (stat != -1) {
            rc = stat;
            listmax = 0;
            buff = lang(u8"なかなかの身体つき"s + _dana() +
                    calcslavevalue(rc) * 2 / 3 + strgold + u8"でどう"s + _da(1),
            ...
            label_2257();
            if (chatval == 1) {
                txt(lang(cdatan(0, rc) + u8"を売り飛ばした。"s,
                ...
                if (cbit(963, rc) == 1) { evadd(15, cdata(27, rc)); }
                del_chara(rc);
                buff = lang(_thanks(2), u8"Thanks!"s);
            } else {
                buff = lang(u8"冷やかし"s + _ka(1), u8"You kidding? "s);
    ...
    if (chatval == 38) {
        if (cdata(17, tc) < 200) {
            buff = lang(u8"("s + name(tc) + u8"はやんわりと断った)"s,
    ...
    if (chatval == 39) {
        ...
        cbitmod(962, tc, 1);
        if (gdata(828) == 0) { gdata(98) = tc; }
        label_2256();
        return;
    }
    if (chatval == 43) {
        listmax = 0;
        buff = lang(u8"悪天候時はシェルターを無料で開放している"s + _nda() +
    ...
    if (chatval == 45) {
        ...
        if (jp) {
            ...
            s(9) = u8"Hovel"s;
            s(10) = u8"Shed"s;
            if (rnd(3)) { mdatan(0) += u8" "s + s(rnd(10)); }
        }
        screenupdate = -1;
        label_1417();
        buff = lang(u8"そう"s + _dana() + u8"これからこの家の名前は"s +
                mdatan(0) + _da(),
    ...
    if (chatval == 46) {
        if (cdata(49, 0) >= -30) {
            buff = lang(u8"その程度の罪なら自分でなんとかしなさい。"s,
                u8"You karma isn't that low. Come back after you have comitted more crimes!"s);
            label_2251();
            return;
        }
        listmax = 0;
        buff = lang(u8"免罪符を希望する"s + _noka(1) + calcguiltvalue() +
                strgold + u8"かかるけどいいの"s + _kana(1),
    ...
    if (chatval == 47) {
        ...
        if (stat != -1) {
            rc = stat;
            listmax = 0;
            buff = lang(u8"10000 goldかかるけどいい"s + _ka(1),
    ...
    if (chatval == 50) {
        buff = lang(_ore(3) + u8"の剣が必要な"s + _noka(1) + u8"そう"s +
                _dana(3) + u8"、"s + calchireadv(tc) + strgold +
    ...
    if (chatval == 51) {
        if (cdata(38, 0) * 3 / 2 + 10 < cdata(38, tc)) {
            buff = lang(_kimi(3) +
                    u8"の仲間になれと？あまりにも力の差がありすぎる"s + _na(),
        ...
        if (cdata(17, tc) >= 200 && cdata(85, tc) > 2) {
            ...
            f = get_freeally();
            if (f == 0) {
                buff = lang(u8"これ以上仲間を連れて行けないよう"s + _da() +
    ...
    if (chatval == 54) {
        if (calccostreload(0) == 0) {
            buff = lang(u8"充填する必要はないみたい"s + _da(),
                u8"Reload what? You don't have any ammo in your inventory."s);
            label_2251();
            return;
        }
        buff = lang(u8"そう"s + _dana(3) + u8"、全ての矢弾を補充すると"s +
    ...
    if (chatval == 56) {
        ...
        ++listmax;
        buff = lang(u8"なかなかの体つき"s + _dana() + u8"よし、買"s + _u(2),
    ...
    if (chatval == 60) {
        ...
        listn(0, listmax) = lang(u8"やめる"s, u8"W-wai.t.."s);
        ++listmax;
        buff = lang(u8"そう"s + _dana() + u8"金貨"s + sexvalue +
                u8"枚を前払いで"s + _kure(),
    ...
    if (chatval >= 10000) {
        ...
        p = dist(cdata(1, 0), cdata(2, 0), cdata(1, rc), cdata(2, rc));
        {
            ...
            for (int cnt_end = cnt + (1); cnt < cnt_end; ++cnt) {
                ...
                if (cdata(0, rc) != 1) {
                    s = lang(u8"奴なら今は死んでいる"s + _yo(2),
                        u8"Oh forget it, dead for now."s);
                    break;
                }
                if (p < 6) {
                    s = lang(cdatan(0, rc) + u8"ならすぐ近くにいる"s + _yo() +
                            s + u8"の方を向いてごらん。"s,
                ...
                if (p < 35) {
                    s = lang(cdatan(0, rc) + u8"に会いたいのなら、"s + s +
                            u8"にかなり歩く必要があ"s + _ru(),
                ...
                s = lang(cdatan(0, rc) + _ka(3) + u8"、ここから"s + s +
