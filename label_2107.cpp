...
int label_2107()
{
    ...
    if (fmode == 8 || fmode == 7) {
        ...
        if (fmode == 8) {
            playerheader = u8""s + cdatan(0, 0) + u8" Lv:"s + cdata(38, 0) + u8" "s + mdatan(0);
            bsave(folder + u8"header.txt"s, playerheader);
        }
        { int cnt = 0; for (int cnt_end = cnt + (15); cnt < cnt_end; ++cnt) {
            procid = (procid + 1) % 4;
            proclist(procid) = u8"ctrlFile:game"s + fread + u8":"s + cnt + u8" ct:"s + ct;
            file = folder;
            if (cnt == 0) {
                file += u8"gdata.s1"s;
                fsize = 4000;
        ...
        procid = (procid + 1) % 4;
        proclist(procid) = u8"ctrlFile:game"s + fread + u8":logs"s + u8" ct:"s + ct;
        ...
        procid = (procid + 1) % 4;
        proclist(procid) = u8"ctrlFile:game"s + fread + u8":array"s + u8" ct:"s + ct;
        ...
        arrayfile();
        procid = (procid + 1) % 4;
        proclist(procid) = u8"ctrlFile:game"s + fread + u8":evs"s + u8" ct:"s + ct;
        if (fread == 0) {
        ...
        procid = (procid + 1) % 4;
        proclist(procid) = u8"ctrlFile:game"s + fread + u8":end"s + u8" ct:"s + ct;
    }
    if (fmode == 14 || fmode == 15) {
        ...
        if (fmode == 14) {
            playerheader = u8""s + cdatan(0, 0) + u8"(Lv"s + cdata(38, 0) + u8")の遺伝子"s;
            file = folder + u8"gene_header.txt"s;
            bsave(file, playerheader);
            fileadd(file);
        }
        { int cnt = 0; for (int cnt_end = cnt + (9); cnt < cnt_end; ++cnt) {
            file = folder;
            if (cnt == 0) {
                file += u8"gene.s1"s;
                fsize = 400;
                ...
                if (cdata(27, tg) == 343) {
                    ...
                    if (strsize != -1) {
                        if (cdata(157, tg) != usernpcmax) {
                            if (instr(untaglist, 0, u8"/"s + userdatan(0, cdata(157, tg)) + u8"/"s) == -1) {
                                if (gdata(86) < 10) {
                                    bcopy(folder + userdatan(6, cdata(157, tg)), folder + u8"_tmp_"s + gdata(86) + u8".npc"s);
                                    zipadd(u8"_tmp_"s + gdata(86) + u8".npc"s);
                                    untaglist += u8"/"s + userdatan(0, cdata(157, tg)) + u8"/"s;
                                    ++gdata(86);
                                }
                            }
                        }
    ...
    if (fmode == 13) {
        procid = (procid + 1) % 4;
        proclist(procid) = u8"ctrlFile"s + fmode + u8" ct:"s + ct;
        { int cnt = 0; for (int cnt_end = cnt + (40); cnt < cnt_end; ++cnt) {
            adata(cnt, area) = 0;
        } }
        folder = exedir + u8"tmp"s;
