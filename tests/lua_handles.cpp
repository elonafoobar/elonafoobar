...
TEST_CASE("Test that handle properties can be read", "[Lua: Handles]")
{
    ...
    SECTION("Characters")
    {
        ...
        REQUIRE_NOTHROW(elona::lua::lua.get_state()->safe_script(R"(assert(chara.index == idx))"));
    ...
    SECTION("Items")
    {
        ...
        elona::lua::lua.get_state()->set("idx", idx);
        REQUIRE_NOTHROW(elona::lua::lua.get_state()->safe_script(R"(assert(item.index == idx))"));
...
TEST_CASE("Test that handles go invalid", "[Lua: Handles]")
{
    ...
    SECTION("Characters")
    {
        ...
        {
            auto result = elona::lua::lua.get_state()->safe_script(R"(print(chara.index))", &sol::script_pass_on_error);
...
TEST_CASE("Test invalid references to handles in store table", "[Lua: Handles]")
{
    ...
    SECTION("Characters")
    {
        ...
        REQUIRE_THROWS(elona::lua::lua.run_in_mod("test", "print(Store.charas[0].index)"));
    }
    SECTION("Items")
    {
        REQUIRE(itemcreate(-1, PUTITORO_PROTO_ID, 4, 8, 3));
        ...
        REQUIRE_THROWS(elona::lua::lua.run_in_mod("test2", "print(Store.items[0].index)"));
    }
}
...
Store.charas = {[0]=chara}
        ...
        REQUIRE_THROWS(elona::lua::lua.run_in_mod("test_invalid_chara", "print(Store.charas[0].index)"));
    }
    SECTION("Items")
    {
...
Store.items = {[0]=items}
        ...
        REQUIRE_THROWS(elona::lua::lua.run_in_mod("test_invalid_item", "print(Store.items[0].index)"));
    }
}
