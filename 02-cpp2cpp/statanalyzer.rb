...
class StatAnalyzer < ASTVisitor
  attr_reader :stat_return
  ...
  def visit_expressionstatement(ast)
    ...
    if FunctionCallAST === ast.expression
      @last_func_call = ast.expression.function.name
    end
  end
  ...
  def visit_variable(ast)
    if ast.name == :stat && @last_func_call
      @stat_return << @last_func_call
    end
  end
