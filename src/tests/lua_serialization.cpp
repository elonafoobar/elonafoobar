...
TEST_CASE("Test that store can be reset", "[Lua: Serialization]")
{
    ...
    REQUIRE_NOTHROW(lua.get_mod_manager().run_in_mod("test", "assert(Store.global.thing == nil)"));
}
...
TEST_CASE("Test that store can be reset across mods", "[Lua: Serialization]")
{
    ...
    REQUIRE_NOTHROW(lua.get_mod_manager().run_in_mod("test1", "assert(Store.global.thing == nil)"));
    REQUIRE_NOTHROW(lua.get_mod_manager().run_in_mod("test1", "assert(Store.global.mine == nil)"));
    REQUIRE_NOTHROW(lua.get_mod_manager().run_in_mod("test2", "assert(Store.global.thing == nil)"));
    REQUIRE_NOTHROW(lua.get_mod_manager().run_in_mod("test2", "assert(Store.global.theirs == nil)"));
}
...
Event.register(Event.EventKind.MapInitialized, my_map_init_hook)
    ...
    REQUIRE_NOTHROW(lua.get_mod_manager().run_in_mod("test", "assert(Store.global.val == 42)"));
...
TEST_CASE("Test preservation of global data", "[Lua: Serialization]")
{
    ...
    REQUIRE_NOTHROW(elona::lua::lua->get_mod_manager().load_mod_from_script("test_serial_global", R"(
local Event = Elona.require("Event")
...
Event.register(Event.EventKind.AllTurnsFinished, my_hook)
    ...
    REQUIRE_NOTHROW(elona::lua::lua->get_mod_manager().run_in_mod("test_serial_global", "assert(Store.global.val == nil)"));
    elona::lua::lua->get_event_manager()
        .run_callbacks<elona::lua::event_kind_t::all_turns_finished>();
    REQUIRE_NOTHROW(elona::lua::lua->get_mod_manager().run_in_mod("test_serial_global", "assert(Store.global.val == 42)"));
...
TEST_CASE("Test preservation of map local data", "[Lua: Serialization]")
{
    ...
    REQUIRE_NOTHROW(elona::lua::lua->get_mod_manager().load_mod_from_script("test_serial_map_local", R"(
local Event = Elona.require("Event")
...
Event.register(Event.EventKind.AllTurnsFinished, my_hook)
    ...
    REQUIRE_NOTHROW(elona::lua::lua->get_mod_manager().run_in_mod("test_serial_map_local", "assert(Store.map_local.val == nil)"));
    elona::lua::lua->get_event_manager()
        .run_callbacks<elona::lua::event_kind_t::all_turns_finished>();
    REQUIRE_NOTHROW(elona::lua::lua->get_mod_manager().run_in_mod("test_serial_map_local", "assert(Store.map_local.val == 42)"));
    ...
    // REQUIRE_NOTHROW(elona::lua::lua->get_mod_manager().run_in_mod("test", "assert(Store.map_local.val == 42)"));
}
