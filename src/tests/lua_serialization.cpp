...
TEST_CASE("Test that store can be reset across mods", "[Lua: Serialization]")
{
    ...
    REQUIRE_NOTHROW(lua.run_in_mod("test2", "assert(Store.global.thing == nil)"));
    REQUIRE_NOTHROW(lua.run_in_mod("test2", "assert(Store.global.theirs == nil)"));
}
...
TEST_CASE("Test preservation of global data", "[Lua: Serialization]")
{
    ...
    REQUIRE_NOTHROW(elona::lua::lua->load_mod_from_script("test_serial_global", R"(
local Event = Elona.require("Event")
...
Event.register(Event.EventKind.AllTurnsFinished, my_hook)
    ...
    REQUIRE_NOTHROW(elona::lua::lua->run_in_mod("test_serial_global", "assert(Store.global.val == nil)"));
    elona::lua::lua->get_event_manager()
        .run_callbacks<elona::lua::event_kind_t::all_turns_finished>();
    REQUIRE_NOTHROW(elona::lua::lua->run_in_mod("test_serial_global", "assert(Store.global.val == 42)"));
...
TEST_CASE("Test preservation of map local data", "[Lua: Serialization]")
{
    ...
    REQUIRE_NOTHROW(elona::lua::lua->load_mod_from_script("test_serial_map_local", R"(
local Event = Elona.require("Event")
...
Event.register(Event.EventKind.AllTurnsFinished, my_hook)
    ...
    REQUIRE_NOTHROW(elona::lua::lua->run_in_mod("test_serial_map_local", "assert(Store.map_local.val == nil)"));
    elona::lua::lua->get_event_manager()
        .run_callbacks<elona::lua::event_kind_t::all_turns_finished>();
    REQUIRE_NOTHROW(elona::lua::lua->run_in_mod("test_serial_map_local", "assert(Store.map_local.val == 42)"));
