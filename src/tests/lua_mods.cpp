...
TEST_CASE("Test that globals cannot be overwritten", "[Lua: Mods]")
{
    ...
    REQUIRE_NOTHROW(mod_mgr.run_in_mod("my_mod", R"(assert(_MOD_NAME == "my_mod"))"));
}
...
TEST_CASE("Test that sandboxing removes unsafe functions", "[Lua: Mods]")
{
    ...
    REQUIRE_THROWS(mod_mgr.run_in_mod("my_mod", R"(rawset(_G, "assert", nil))"));
    ...
    REQUIRE_THROWS(mod_mgr.run_in_mod("my_mod", R"(dofile("tests/lua/item.lua"))"));
    REQUIRE_THROWS(mod_mgr.run_in_mod("my_mod", R"(loadfile("tests/lua/item.lua"))"));
    REQUIRE_THROWS(mod_mgr.run_in_mod("my_mod", R"(load("return function(a,b) return a+b end"))"));
    REQUIRE_THROWS(mod_mgr.run_in_mod("my_mod", R"(setmetatable(_G, {}))"));
    REQUIRE_THROWS(mod_mgr.run_in_mod("my_mod", R"(getmetatable(_G))"));
    REQUIRE_THROWS(mod_mgr.run_in_mod("my_mod", R"(module(..., package.seeall))"));
    ...
    REQUIRE_THROWS(mod_mgr.run_in_mod("my_mod", R"(package.loadlib("LibSDL2.dll", "main")())"));
...
TEST_CASE("Test usage of store in mod", "[Lua: Mods]")
{
    ...
    REQUIRE_NOTHROW(mod_mgr.load_mod_from_script("test", "Store.global.thing = 1"));

    REQUIRE_NOTHROW(mod_mgr.run_in_mod("test", "assert(Store.global.thing == 1)"));
...
Event.register(Event.EventKind.AllTurnsFinished, my_turn_handler)
    ...
    REQUIRE_NOTHROW(mod_mgr.run_in_mod("test", "assert(Store.global.thing == 2)"));

    lua.get_event_manager()
        .run_callbacks<elona::lua::event_kind_t::all_turns_finished>();
    lua.get_event_manager()
        .run_callbacks<elona::lua::event_kind_t::all_turns_finished>();
    REQUIRE_NOTHROW(mod_mgr.run_in_mod("test", "assert(Store.global.thing == 4)"));
}
...
TEST_CASE("Test isolation of mod environments", "[Lua: Mods]")
{
    ...
    REQUIRE_NOTHROW(mod_mgr.load_mod_from_script("first", R"(Store.global.thing = 42)"));
    REQUIRE_NOTHROW(mod_mgr.load_mod_from_script("second", R"(Store.global.thing = "dood")"));

    REQUIRE_NOTHROW(mod_mgr.run_in_mod("first", R"(assert(Store.global.thing == 42))"));
    REQUIRE_NOTHROW(mod_mgr.run_in_mod("second", R"(assert(Store.global.thing == "dood"))"));

    REQUIRE_NOTHROW(mod_mgr.run_in_mod("second", R"(Store.global.thing = false)"));

    REQUIRE_NOTHROW(mod_mgr.run_in_mod("first", R"(assert(Store.global.thing == 42))"));
    REQUIRE_NOTHROW(mod_mgr.run_in_mod("second", R"(assert(Store.global.thing == false))"));
}
...
Event.register(Event.EventKind.AllTurnsFinished, my_turn_handler)
    ...
    REQUIRE_NOTHROW(mod_mgr.run_in_mod("test", "assert(Store.global.grid[1][1] == 0)"));

    lua.get_event_manager()
        .run_callbacks<elona::lua::event_kind_t::all_turns_finished>();
    REQUIRE_NOTHROW(mod_mgr.run_in_mod("test", "assert(Store.global.grid[1][1] == 1)"));
}
