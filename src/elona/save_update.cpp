...
std::string _update_save_data_5_convert_portrait_id(int old_id, int sex)
{
    ...
    case 54: return sex ? "core.starsha" : "core.saimore";
...
void _update_save_data_5(const fs::path& save_dir, int serial_id)
{
    ...
    // Convert integer portrait ids to string ones.
    for (const auto& entry : filesystem::dir_entries(
             save_dir,
             filesystem::DirEntryRange::Type::file,
             std::regex{u8R"((g_)?cdata(_.*)?\.s[12])"}))
...
/// Update save data from v8 to v9. Convert integer character ids to string
/// character ids.
void _update_save_data_8(const fs::path& save_dir, int serial_id)
{
             ...
             std::regex{u8R"((g_)?cdata(_.*)?\.s[12])"}))
    {
        ...
        for (int idx = begin; idx < end; ++idx)
        {
            ...
            // Convert character ids.
            CharacterId new_id;
            {
                new_id = id;
                ELONA_LOG(
                    "[Save data] Convert character ID of character("
                    << idx << "): " << id << " => " << new_id.get());
            }
            // Dump character data to the memory stream.
            {
