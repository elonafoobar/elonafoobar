...
void do_play_scene()
{
    ...
    scenemode = 1;
    SDIM4(actor, 20, 3, 10);
    data::InstanceId background_image_id{"core.void"};
    y1 = 60;
    y2 = windowh - 60;
    notesel(buff);
    {
    ...
    while (true)
    {
        ...
        p(1) = instr(buff, scidx, "{"s) + scidx;
        if (p(1) == -1)
        {
            scene_fade_to_black();
            return;
        }
        p(2) = instr(buff, scidx, "}"s) + scidx + 1;
        if (p(2) == -1)
        {
            scene_fade_to_black();
            return;
        ...
        p(3) = instr(buff, scidx, "\""s) + scidx + 1;
        p(4) = instr(buff, p(3), "\""s) + p(3);
        ...
        if (strutil::contains(s(0), "{actor_"))
        {
...
void conquer_lesimas()
{
    ...
    if (jp)
    {
        ...
        txt("「お前がここに辿り着くことは」台座から、何かの声が聞こえる。"s);
        flt();
        chara_create(
            -1,
            "core.orphe",
        ...
        msg_halt();
        Message::instance().clear();
        txt("「決まっていたことなのだ…遅かれ早かれな」"s);
        txt("部屋の空気が突然緊張し、あなたの前に端麗な青年が現れた。"s);
        txt("「我々からすれば、複雑性の一面に過ぎないが、人間は運命とでも呼ぶのだろう？」"s);
        msg_halt();
        Message::instance().clear();
        txt("あなたは懸命に脚の震えを抑えようとしたが、難しかった。"s);
        txt("華奢に見える幼顔の男の影は、人のものではない。"s);
        txt("あどけない瞳の奥に、あなたは底知れない力と闇を感じた。"s);
        msg_halt();
        Message::instance().clear();
        txt("「ネフィアの永遠の盟約に基づき」青年は台座の横の死体を指し、皮肉な微笑を送った。"s);
        txt("「この哀れな老人が守っていたものは、今からお前のものだ」"s);
        msg_halt();
        Message::instance().clear();
        txt("あなたは、台座の上に置かれている絢爛な装飾の本を、いぶかしげに眺めた。"s);
        msg_halt();
