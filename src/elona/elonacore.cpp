...
void initialize_rankn()
{
    ...
    for (int category = 0; category < 9; category++)
    {
        ...
        {
            rankn(rank, category) =
                i18n::s.get_enum("core.locale.rank._"s + category, rank);
        }
    }
...
void character_drops_item()
{
        ...
        cdata[rc].character_role != 20)
    {
            ...
            (cdata[rc].quality == Quality::godly && rnd(3) == 0))
        {
            ...
            inv[ci].subname = cdata[rc].id.to_integer();
...
void lovemiracle(int chara_index)
{
    ...
    if (rnd(2))
    {
        ...
        if (stat)
        {
            inv[ci].subname = cdata[chara_index].id.to_integer();
...
int gain_skills_by_geen_engineering()
{
    ...
    randomize(cdata[tc].id.to_integer());
    int dbmax = 0;
    for (int cnt = 0; cnt < 100; ++cnt)
    {
        rtval = rnd(40) + 150;
