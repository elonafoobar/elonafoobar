...
void UIMenuMods::_load_mods()
{
    ...
    if (_is_download)
    {
        ...
        DownloadModsInitPrompt(
            i18n::s.get("core.locale.main_menu.mod_list.download.failed"))
            .query();
        _is_download = false;
        _load_mods();
    }
...
void UIMenuMods::_draw_window()
{
    auto hint = ""s + key_enter + " " +
        i18n::s.get("core.locale.main_menu.mod_list.hint.toggle") + "  " +
        key_identify + " " +
        i18n::s.get("core.locale.main_menu.mod_list.hint.info");

    if (_is_download)
    {
        hint += "  " + key_mode + " " +
            i18n::s.get("core.locale.main_menu.mod_list.hint.installed");
    }
    else
    {
        hint += "  " + key_mode + " " +
            i18n::s.get("core.locale.main_menu.mod_list.hint.download");
    }
    ...
    if (_is_download)
    {
        title = i18n::s.get("core.locale.main_menu.mod_list.title.download");
    }
    else
    {
        title = i18n::s.get("core.locale.main_menu.mod_list.title.installed");
    }
    ...
    display_topic(
        i18n::s.get("core.locale.main_menu.mod_list.info.name"),
        wx + 46,
        wy + 36);
    display_topic(
        i18n::s.get("core.locale.main_menu.mod_list.info.version"),
        wx + 255,
        wy + 36);
}
