    ...
    int& p)
{
    ...
    list(0, p) = ItemDescriptionType::normal;
    listn(0, p) = "";
    ++p;
    notesel(buf);
    for (int cnt = 0, cnt_end = (noteinfo()); cnt < cnt_end; ++cnt)
    {
        ...
        if (strlen_u(desc_line) > max_width)
        {
            ...
            for (size_t i = 0; i < strlen_u(desc_line) / max_width + 1; ++i)
            {
                ...
                list(0, p) = ItemDescriptionType::small_font;
                listn(0, p) = one_line;
                ++p;
        ...
        else
        {
            ...
            if (cnt == noteinfo() - 1)
            {
                list(0, p) = ItemDescriptionType::small_font_italic;
            }
            ++p;
        }
    }
    ...
    int& p)
{
    ...
    {
        list(0, p) = ItemDescriptionType::text;
        listn(0, p) = *text;
        ++p;
    }
}
...
static void _load_item_stat_text(int ci, int& p)
{
    ...
    if (inv[ci].dice_x != 0)
    {
        ...
        list(0, p) = ItemDescriptionType::weapon_info;
        listn(0, p) =
    ...
    {
        list(0, p) = ItemDescriptionType::armor_info;
        listn(0, p) =
...
static void _load_item_enchantment_desc(int ci, int& p)
{
    ...
    if (ibit(15, ci))
    {
        list(0, p) = ItemDescriptionType::enchantment;
