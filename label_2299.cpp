...
{
    if (dbmode != 1) {
        stat=label_2654();return;
        return;
    }
    dbsum = 0;
    dbmax = 0;
    if (1 > objlv) {
        goto label_2300_internal;
    }
    if (fltselect != 1) {
        goto label_2300_internal;
    }
    ...
    if (fltnrace != u8""s) {
        if (fltnrace != u8"slime"s) {
            goto label_2300_internal;
        }
    }
    if (filtermax != 0) {
        dbtmp = 1;
        { int cnt = 0; for (int cnt_end = cnt + (filtermax); cnt < cnt_end; ++cnt) {
...
label_2652_internal:
    ...
    ++dbmax;
label_2653_internal:
    { int cnt = 0; for (int cnt_end = cnt + (usernpcmax); cnt < cnt_end; ++cnt) {
        if (cmshade) {
            break;
        }
        if (fltselect != userdata(12, cnt)) {
            continue;
        }
        if (flttypemajor != 0) {
            continue;
        ...
        if (fltnrace != u8""s) {
            if (fltnrace != userdatan(2, cnt)) {
                continue;
            }
        }
        if (userdata(2, cnt) > objlv) {
            continue;
        }
        if (filtermax != 0) {
            dbtmp = 1;
        ...
        dbsum += limit(userdata(6, cnt), 1, 500) * 1000 / (500 + abs((objlv - userdata(2, cnt))) * 400) + 1;
        unlist(dbmax) = cnt;
        dblist(0, dbmax) = 343;
        dblist(1, dbmax) = dbsum;
        ++dbmax;
    ...
    if (dbsum != 0) {
        ...
        { int cnt = 0; for (int cnt_end = cnt + (dbmax); cnt < cnt_end; ++cnt) {
