...
void cell_draw()
{
                    ...
                    if (map(x_, y_, 5) != 0)
                    {
                        ...
                        label_0380_internal:
                            ...
                            else
                            {
                                ...
                                gfini(chipi(2, p_), chipi(3, p_));
                                gfdec2(
                                    c_col(0, i_), c_col(1, i_), c_col(2, i_));
                                gsel(selcur);
                            }
                            if (mdata(6) == 1)
                            {
                    ...
                    if (map(x_, y_, 1) != 0)
                    {
                        ...
                        if (synccheck(c_, -1))
                        {
                            ...
                            if (c_ != 0)
                            {
                                ...
                                {
                                    h_ = std::min(
                                        cdata[c_].hp * 30 / cdata[c_].max_hp,
                                        30);
                                    if (h_ > 0)
                                    {
                                        if (c_ < 16)
                            ...
                            || lightdata(6, p_))
                        {
                            if (mapsync(x_, y_) == msync)
                            {
                                ...
                                gmode(
                                    ...
                                    inf_tiles,
                                    lightdata(2, p_)
                                        + rnd((lightdata(5, p_) + 1)));
                                gcopy(
                                    3,
                                    192 + lightdata(0, p_) * 48
                                        + rnd((lightdata(1, p_) + 1))
                                            * inf_tiles,
                                    704);
                            }
                        }
    ...
    {
        color(0, 0, 0);
        gmode(6, inf_tiles, inf_tiles, light_);
        y_ = 2;
        dy_ = inf_screeny;
        {
            int cnt = 0;
    ...
    {
        y_ = 2;
        dy_ = inf_screeny + syfix_ * (scrollp > 3);
        {
            int cnt = 0;
            for (int cnt_end = cnt + (inf_screenh); cnt < cnt_end; ++cnt)
            {
                x_ = 2;
                dx_ = inf_screenx + sxfix_ * (scrollp > 3);
                cnt2_ = cnt;
                {
                    int cnt = 0;
                    for (int cnt_end = cnt + (inf_screenw); cnt < cnt_end;
