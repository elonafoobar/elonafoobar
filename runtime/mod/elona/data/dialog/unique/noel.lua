...
local function buy_nuke()
   ...
   Item.create(Chara.player().position, "elona.nuclear_bomb", 0)
end

return {
   id = "noel",
   root = "elona.talk.unique.noel",
   nodes = {
      __start = function()
      ...
      quest_finish = {
         text = {
            function()
               local secret_treasure = Item.create(Chara.player().position, {id = "elona.secret_treasure", nostack = true})
               secret_treasure.param1 = 162
               Item.create(Chara.player().position, "elona.platinum_coin", 6)
