...
local function take_books()
   ...
   for _, item in Item.iter(0, 200) do
      if item.number ~= 0 and item.id == "elona.book_of_rachel" and item.param2 >= 1 and item.param2 <= 4 then
...
return {
   ...
   nodes = {
      ...
      quest_finish = {
         ...
         on_finish = function()
            Item.create(Chara.player().position, "elona.statue_of_lulwy", 0)
            Item.create(Chara.player().position, "elona.hero_cheese", 0)
            Item.create(Chara.player().position, "elona.gold_piece", 20000)
            Item.create(Chara.player().position, "elona.platinum_coin", 5)
