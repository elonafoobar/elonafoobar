...
local function give_monster_balls()
   ...
   for _, item in Item.iter(0, 200) do
      ...
      if item.number > 0 and item.id == "elona.monster_ball" and item.subname ~= 0 then
         found = true
...
return {
   ...
   nodes = {
      ...
      quest_yes = {
         text = {
            function()
               local monster_balls = Item.create(Chara.player().position, {id = "elona.monster_ball", nostack = true, number = 6})
      ...
      quest_finish = {
         ...
         on_finish = function()
            Item.create(Chara.player().position, "elona.gene_machine", 0)
            Item.create(Chara.player().position, "elona.gold_piece", 2500)
            Item.create(Chara.player().position, "elona.platinum_coin", 2)
