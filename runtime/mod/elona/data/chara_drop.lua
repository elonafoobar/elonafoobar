...
local function lootrich(chara, count)
   ...
   if Rand.one_in(3) then
      Item.create(chara.position, "elona.wallet")
   end
end
...
data:add_multi(
   "core.chara_drop",
   {
      ...
      {
         id = "vesda",
         drops = {{ id = "elona.gloves_of_vesda" }}
      },
      {
         id = "god_boss",
         on_generate = function(args)
            ...
            if Chara.kill_count(args.chara.id) == 1 then
               return make_drops({ "elona.rod_of_wishing", "elona.artifact_seed" })
            else
               return make_drops({ "elona.artifact_seed",  "elona.artifact_seed" })
            end
         end
      },
      {
         ...
         on_generate = function()
            if Map.id() == "elona.pyramid" then
               return make_drops({ "elona.statue_of_opatos",
                                   "elona.platinum_coin",
                                   "elona.platinum_coin",
                                   "elona.platinum_coin",
                                   "elona.platinum_coin",
      ...
      {
         id = "rabbit",
         drops = {{ one_in = 200, id = "elona.rabbits_tail" }}
      },
      {
         id = "zombie",
         drops = {{ one_in = 100, id = "elona.potion_of_descent" }}
      },
      {
         id = "lomias",
         drops = {{ one_in = 4,   id = "elona.secret_experience_of_lomias" }}
      },
      {
         id = "mummy",
         drops = {{ one_in = 300, id = "elona.book_of_resurrection" }}
      },
      {
         id = "cupid_of_love",
         drops = {{ one_in = 140, id = "elona.happy_bed" }}
      },
      {
         id = "hermit_crab",
         drops = {{ one_in = 100, id = "elona.solemn_tomb" }}
      },
      {
         id = "rogue",
         drops = {{ one_in = 90, id = "elona.blue_capsule_drug" }}
      },
      {
         id = "executioner",
         drops = {{ one_in = 200, id = "elona.rabbits_tail" }}
      ...
      {
         id = "deformed_eye",
         drops = {{ one_in = 150, id = "elona.potion_of_evolution" }}
      },
      {
         id = "gwen",
         drops = {{
               one_in = 50,
               on_create = function(args)
                  local item = Item.create(args.chara.position, "elona.secret_treasure")
                  if item then
                     item.param1 = 169
                  end
               end
      ...
      {
         id = "fairy",
         drops = {{ one_in = 200, id = "elona.secret_experience_of_kumiromi" }}
      },
      {
         id = "mine_dog",
         drops = {{ one_in = 3, id = "elona.mine" }}
      },
      {
         id = "imp",
         drops = {{ one_in = 600, id = "elona.scroll_of_name" }}
      },
      {
         id = "rogue_boss",
         drops = {{
      ...
      },
      {
         ...
         drops = {{
               on_create = function(args)
                  Item.create(args.chara.position, "elona.platinum_coin", 1 + Rand.rnd(3))
                  if Rand.coinflip() then
                     Item.create(args.chara.position, "elona.small_medal")
                  end
               end
         }}
      },
      {
         ...
         drops = {{
               on_create = function(args)
                  Item.create(args.chara.position, "elona.gold_piece", 2500 + Rand.rnd_capped(Chara.player().fame + 1000))
               end
         }}
      },
      {
