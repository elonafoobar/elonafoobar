data:add_multi(
   "core.chara",
   {
      {
         id = "bug",
         legacy_id = 0,
         item_type = 3,
         tags = {},
         level = 1,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.slime",
         class_ = "elona.predator",
         cspecialeq = 0,
         fltselect = 1,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "user",
         legacy_id = 343,
         item_type = 6,
         tags = {},
         level = 1,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Neutral",
         race = "elona.god",
         class_ = "elona.gunner",
         sex = "Male",
         image = 1,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "shopkeeper",
         legacy_id = 1,
         item_type = 3,
         tags = { "man" },
         level = 35,
         portrait = "__random__",
         ai_calm = 2,
         ai_move = 80,
         ai_dist = 6,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.gunner",
         male_image = 140,
         female_image = 141,
         cspecialeq = 0,
         fltselect = 7,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "caravan_master",
         legacy_id = 353,
         item_type = 3,
         tags = { "man" },
         level = 22,
         portrait = "__random__",
         ai_calm = 3,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         has_random_name = true,
         original_relationship = "Neutral",
         race = "elona.juere",
         class_ = "elona.gunner",
         image = 443,
         cspecialeq = 0,
         fltselect = 1,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "bartender",
         legacy_id = 70,
         item_type = 3,
         tags = { "man" },
         level = 20,
         portrait = "__random__",
         ai_calm = 2,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 4,
         has_random_name = true,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.gunner",
         male_image = 144,
         female_image = 145,
         cspecialeq = 0,
         fltselect = 1,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "informer",
         legacy_id = 69,
         item_type = 3,
         tags = { "man" },
         level = 20,
         portrait = "__random__",
         ai_calm = 2,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         has_random_name = true,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.warrior",
         male_image = 15,
         female_image = 16,
         cspecialeq = 0,
         fltselect = 7,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "arena_master",
         legacy_id = 73,
         item_type = 3,
         tags = { "man" },
         level = 40,
         portrait = "__random__",
         ai_calm = 2,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         has_random_name = true,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.warrior",
         sex = "Male",
         image = 41,
         cspecialeq = 0,
         fltselect = 7,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "healer",
         legacy_id = 74,
         item_type = 3,
         tags = { "man" },
         level = 20,
         portrait = "__random__",
         ai_calm = 2,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         has_random_name = true,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.warrior",
         sex = "Female",
         image = 69,
         cspecialeq = 0,
         fltselect = 7,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.holy_one",
      },
      {
         id = "nun",
         legacy_id = 206,
         item_type = 3,
         tags = { "man" },
         level = 50,
         portrait = "__random__",
         ai_calm = 2,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 4,
         has_random_name = true,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.wizard",
         sex = "Female",
         image = 163,
         cspecialeq = 0,
         fltselect = 7,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.holy_one",
      },
      {
         id = "elder",
         legacy_id = 38,
         item_type = 3,
         tags = { "man" },
         level = 20,
         portrait = "__random__",
         ai_calm = 2,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         has_random_name = true,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.gunner",
         male_image = 142,
         female_image = 143,
         cspecialeq = 0,
         fltselect = 7,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "trainer",
         legacy_id = 40,
         item_type = 3,
         tags = { "man" },
         level = 40,
         portrait = "__random__",
         ai_calm = 2,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         has_random_name = true,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.warrior",
         male_image = 7,
         female_image = 10,
         cspecialeq = 0,
         fltselect = 7,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "guild_trainer",
         legacy_id = 333,
         item_type = 3,
         tags = { "man" },
         level = 69,
         portrait = "__random__",
         ai_calm = 2,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         has_random_name = true,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.warrior",
         male_image = 415,
         female_image = 414,
         cspecialeq = 0,
         fltselect = 1,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "guard_port_kapul",
         legacy_id = 76,
         item_type = 3,
         tags = { "man" },
         level = 40,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 50,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 2,
         has_random_name = true,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.archer",
         image = 363,
         cspecialeq = 0,
         fltselect = 9,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.guard",
      },
      {
         id = "guard",
         legacy_id = 77,
         item_type = 3,
         tags = { "man" },
         level = 40,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 50,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 2,
         has_random_name = true,
         original_relationship = "Neutral",
         race = "elona.yerles",
         class_ = "elona.archer",
         image = 25,
         cspecialeq = 0,
         fltselect = 9,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.guard",
      },
      {
         id = "palmian_elite_soldier",
         legacy_id = 204,
         item_type = 3,
         tags = { "man" },
         level = 10,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 50,
         ai_dist = 2,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -3, -3, -2 },
         can_talk = 2,
         has_random_name = true,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.gunner",
         male_image = 74,
         female_image = 75,
         cspecialeq = 0,
         fltselect = 9,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "zeome",
         legacy_id = 2,
         item_type = 3,
         tags = { "man" },
         level = 55,
         portrait = "elona.zeome",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 20,
         creaturepack = 0,
         normal_actions = { 414, -1, 433 },
         special_actions = { 424 },
         can_talk = 4,
         ai_heal = 403,
         original_relationship = "Aggressive",
         race = "elona.elea",
         class_ = "elona.warmage",
         sex = "Male",
         image = 297,
         fixlv = 6,
         cspecialeq = 1,
         eqweapon1 = 63,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         dialog_id = "elona.zeome",
      },
      {
         id = "at",
         legacy_id = 37,
         item_type = 3,
         tags = { "man" },
         level = 1,
         portrait = "elona.zeome",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 4,
         original_relationship = "Aggressive",
         race = "elona.norland",
         class_ = "elona.predator",
         sex = "Male",
         image = 347,
         cspecialeq = 0,
         fltselect = 1,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.at",
      },
      {
         id = "orphe",
         legacy_id = 23,
         item_type = 3,
         tags = { "man", "god" },
         level = 20,
         portrait = "elona.orphe",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 4,
         original_relationship = "Neutral",
         race = "elona.juere",
         class_ = "elona.warrior",
         sex = "Male",
         image = 331,
         fixlv = 6,
         cspecialeq = 1,
         eqweapon1 = 64,
         fltselect = 3,
         category = 0,
         rarity = 1000,
         coefficient = 400,
         dialog_id = "elona.orphe",
      },
      {
         id = "mad_scientist",
         legacy_id = 26,
         item_type = 3,
         tags = { "man" },
         level = 6,
         portrait = "elona.man7",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.norland",
         class_ = "elona.wizard",
         sex = "Male",
         image = 332,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 2,
         category = 0,
         rarity = 1000,
         coefficient = 400,
      },
      {
         id = "isca",
         legacy_id = 27,
         item_type = 3,
         tags = { "man", "god" },
         level = 42,
         portrait = "elona.isca",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.norland",
         class_ = "elona.archer",
         sex = "Female",
         image = 333,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 2,
         category = 0,
         rarity = 1000,
         coefficient = 400,
      },
      {
         id = "whom_dwell_in_the_vanity",
         legacy_id = 28,
         item_type = 3,
         tags = { "man" },
         level = 78,
         portrait = "elona.bethel",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 4,
         original_relationship = "Neutral",
         race = "elona.juere",
         class_ = "elona.warrior",
         sex = "Male",
         image = 334,
         fixlv = 6,
         cspecialeq = 1,
         eqweapon1 = 73,
         fltselect = 3,
         category = 0,
         rarity = 1000,
         coefficient = 400,
         dialog_id = "elona.whom_dwell_in_the_vanity"
      },
      {
         id = "loyter",
         legacy_id = 29,
         item_type = 3,
         tags = { "man" },
         level = 50,
         portrait = "elona.loyter",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 4,
         original_relationship = "Neutral",
         race = "elona.juere",
         class_ = "elona.warrior",
         sex = "Male",
         image = 337,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 1000,
         coefficient = 400,
         dialog_id = "elona.loyter",
      },
      {
         id = "vesda",
         legacy_id = 140,
         item_type = 5,
         tags = { "dragon" },
         level = 25,
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 1,
         ai_act_sub_freq = 15,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 602 },
         original_relationship = "Aggressive",
         race = "elona.dragon",
         class_ = "elona.predator",
         resistances = {
            ["elona.element_fire"] = 500,
         },
         image = 338,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 1000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.vesda",
         drops = { "elona.vesda" },
      },
      {
         id = "miches",
         legacy_id = 30,
         item_type = 3,
         tags = { "man" },
         level = 20,
         portrait = "elona.miches",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 4,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.archer",
         sex = "Female",
         image = 8,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         dialog_id = "elona.miches",
      },
      {
         id = "shena",
         legacy_id = 31,
         item_type = 3,
         tags = { "man" },
         level = 20,
         portrait = "elona.shena",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 4,
         original_relationship = "Neutral",
         race = "elona.juere",
         class_ = "elona.warrior",
         sex = "Female",
         image = 109,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         dialog_id = "elona.shena",
      },
      {
         id = "the_leopard_warrior",
         legacy_id = 351,
         item_type = 3,
         tags = { "man" },
         level = 130,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 2,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 647 },
         can_talk = 4,
         original_relationship = "Neutral",
         race = "elona.cat",
         class_ = "elona.warrior",
         sex = "Male",
         image = 478,
         fixlv = 6,
         cspecialeq = 1,
         eqtwohand = 1,
         eqweapon1 = 232,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsQuickTempered" },
         drops = { "elona.the_leopard_warrior" },
      },
      {
         id = "silvia",
         legacy_id = 352,
         item_type = 3,
         tags = { "man" },
         level = 1,
         ai_calm = 5,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 4,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.tourist",
         sex = "Female",
         image = 479,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         drops = { "elona.the_leopard_warrior" },
      },
      {
         id = "dungeon_cleaner",
         legacy_id = 32,
         item_type = 3,
         tags = {},
         level = 20,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 4,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.warrior",
         sex = "Male",
         image = 344,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "larnneire",
         legacy_id = 33,
         item_type = 3,
         tags = { "man" },
         level = 20,
         portrait = "elona.larnneire",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 25,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 416 },
         can_talk = 4,
         original_relationship = "Neutral",
         race = "elona.elea",
         class_ = "elona.warmage",
         sex = "Female",
         image = 345,
         fixlv = 6,
         cspecialeq = 1,
         eqweapon1 = 206,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         dialog_id = "elona.larnneire",
      },
      {
         id = "lomias",
         legacy_id = 34,
         item_type = 3,
         tags = { "man" },
         level = 20,
         portrait = "elona.lomias",
         ai_calm = 1,
         ai_move = 60,
         ai_dist = 3,
         ai_act_sub_freq = 15,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 447 },
         can_talk = 4,
         original_relationship = "Neutral",
         race = "elona.elea",
         class_ = "elona.archer",
         sex = "Male",
         image = 346,
         fixlv = 6,
         cspecialeq = 1,
         eqweapon1 = 1,
         eqrange_0 = 207,
         eqammo_0 = 25001,
         eqammo_1 = 3,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         drops = { "elona.lomias" },
         dialog_id = "elona.lomias",
      },
      {
         id = "slan",
         legacy_id = 139,
         item_type = 3,
         tags = { "man" },
         level = 10,
         portrait = "elona.karam",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.warrior",
         sex = "Male",
         image = 99,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         dialog_id = "elona.slan",
      },
      {
         id = "karam",
         legacy_id = 146,
         item_type = 3,
         tags = { "man" },
         level = 10,
         portrait = "elona.karam",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.warrior",
         sex = "Male",
         image = 99,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         dialog_id = "elona.karam",
      },
      {
         id = "erystia",
         legacy_id = 142,
         item_type = 3,
         tags = { "man" },
         level = 10,
         portrait = "elona.erystia",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 4,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.wizard",
         sex = "Female",
         image = 340,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         dialog_id = "elona.erystia",
      },
      {
         id = "issizzle",
         legacy_id = 141,
         item_type = 6,
         tags = { "undead", "god" },
         level = 28,
         ai_calm = 1,
         ai_move = 60,
         ai_dist = 1,
         ai_act_sub_freq = 30,
         creaturepack = 0,
         normal_actions = { -1, 414, 419, 422 },
         special_actions = { 410, 443 },
         original_relationship = "Aggressive",
         race = "elona.lich",
         class_ = "elona.wizard",
         image = 339,
         fixlv = 6,
         cspecialeq = 1,
         eqweapon1 = 358,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.insanity",
      },
      {
         id = "wynan",
         legacy_id = 143,
         item_type = 3,
         tags = {},
         level = 25,
         ai_calm = 1,
         ai_move = 60,
         ai_dist = 1,
         ai_act_sub_freq = 30,
         creaturepack = 0,
         normal_actions = { -1, 414, 419, 422 },
         special_actions = { 410 },
         original_relationship = "Aggressive",
         race = "elona.norland",
         class_ = "elona.warrior",
         image = 341,
         fixlv = 6,
         cspecialeq = 1,
         eqweapon1 = 359,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "quruiza",
         legacy_id = 144,
         item_type = 6,
         tags = {},
         level = 24,
         ai_calm = 1,
         ai_move = 60,
         ai_dist = 1,
         ai_act_sub_freq = 30,
         creaturepack = 0,
         normal_actions = { -1, 414, 419, 422 },
         special_actions = { 410 },
         original_relationship = "Aggressive",
         race = "elona.norland",
         class_ = "elona.wizard",
         resistances = {
            ["elona.element_fire"] = 500,
         },
         image = 343,
         fixlv = 6,
         cspecialeq = 1,
         eqweapon1 = 356,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "corgon",
         legacy_id = 145,
         item_type = 5,
         tags = {},
         level = 16,
         ai_calm = 1,
         ai_move = 60,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.dragon",
         resistances = {
            ["elona.element_fire"] = 500,
         },
         image = 342,
         fixlv = 6,
         cspecialeq = 1,
         eqring1 = 357,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "lulwy",
         legacy_id = 306,
         item_type = 6,
         tags = { "god" },
         level = 350,
         ai_calm = 1,
         ai_move = 60,
         ai_dist = 2,
         ai_act_sub_freq = 30,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 648, 636 },
         original_relationship = "Neutral",
         race = "elona.god",
         class_ = "elona.archer",
         sex = "Female",
         image = 393,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "ehekatl",
         legacy_id = 331,
         item_type = 6,
         tags = { "god" },
         level = 350,
         ai_calm = 1,
         ai_move = 60,
         ai_dist = 2,
         ai_act_sub_freq = 15,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 657 },
         original_relationship = "Neutral",
         race = "elona.god",
         class_ = "elona.warmage",
         sex = "Female",
         image = 413,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "god_inside_ehekatl",
         legacy_id = 336,
         item_type = 6,
         tags = { "god" },
         level = 1200,
         ai_calm = 1,
         ai_move = 30,
         ai_dist = 2,
         ai_act_sub_freq = 25,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 612, 459, 620, 601 },
         original_relationship = "Aggressive",
         race = "elona.god",
         class_ = "elona.warmage",
         sex = "Female",
         image = 413,
         fixlv = 6,
         cspecialeq = 1,
         eqtwohand = 1,
         eqweapon1 = 739,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "opatos",
         legacy_id = 338,
         item_type = 6,
         tags = { "god" },
         level = 350,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 25,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 657 },
         original_relationship = "Neutral",
         race = "elona.god",
         class_ = "elona.warrior",
         sex = "Male",
         image = 432,
         fixlv = 6,
         cspecialeq = 1,
         eqtwohand = 1,
         eqweapon1 = 739,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "kumiromi",
         legacy_id = 339,
         item_type = 6,
         tags = { "god" },
         level = 350,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 25,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 657 },
         original_relationship = "Neutral",
         race = "elona.god",
         class_ = "elona.farmer",
         sex = "Male",
         image = 433,
         fixlv = 6,
         cspecialeq = 1,
         eqtwohand = 1,
         eqweapon1 = 739,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "mani",
         legacy_id = 342,
         item_type = 6,
         tags = { "god" },
         level = 350,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 25,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 657 },
         original_relationship = "Neutral",
         race = "elona.god",
         class_ = "elona.gunner",
         sex = "Male",
         image = 447,
         fixlv = 6,
         cspecialeq = 1,
         eqtwohand = 1,
         eqweapon1 = 739,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "_test",
         legacy_id = 340,
         item_type = 6,
         tags = {},
         level = 1200,
         ai_calm = 1,
         ai_move = 30,
         ai_dist = 2,
         ai_act_sub_freq = 25,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 612, 459, 620, 601 },
         original_relationship = "Aggressive",
         race = "elona.god",
         class_ = "elona.warmage",
         sex = "Male",
         image = 435,
         fixlv = 6,
         cspecialeq = 1,
         eqtwohand = 1,
         eqweapon1 = 739,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "putit",
         legacy_id = 3,
         item_type = 1,
         tags = { "slime" },
         level = 1,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 3,
         normal_actions = { -1 },
         can_talk = 1,
         original_relationship = "Aggressive",
         race = "elona.slime",
         class_ = "elona.tourist",
         resistances = {
            -- Element acid is invalid in vanilla, although it is defined.
            -- ["elona.element_acid"] = 500,
         },
         cspecialeq = 0,
         fltselect = 0,
         category = 3,
         rarity = 80000,
         coefficient = 400,
         flags = { "IsSuitableForMount" },
         corpse_eating_callback = "elona.eating_effect.putit",
      },
      {
         id = "red_putit",
         legacy_id = 4,
         item_type = 1,
         tags = { "fire", "slime" },
         level = 4,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 3,
         normal_actions = { -1 },
         can_talk = 1,
         original_relationship = "Aggressive",
         race = "elona.slime",
         resistances = {
            -- Element acid is invalid in vanilla, although it is defined.
            -- ["elona.element_acid"] = 500,
         },
         color = "Red",
         cspecialeq = 0,
         fltselect = 0,
         category = 3,
         rarity = 70000,
         coefficient = 400,
         flags = { "IsSuitableForMount" },
         corpse_eating_callback = "elona.eating_effect.putit",
      },
      {
         id = "slime",
         legacy_id = 169,
         item_type = 1,
         tags = { "slime" },
         level = 10,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 3,
         normal_actions = { -1 },
         can_talk = 1,
         original_relationship = "Aggressive",
         race = "elona.slime",
         class_ = "elona.predator",
         resistances = {
            -- Element acid is invalid in vanilla, although it is defined.
            -- ["elona.element_acid"] = 500,
         },
         image = 257,
         color = "Blue",
         cspecialeq = 0,
         damage_reaction_info = 100063,
         fltselect = 0,
         category = 3,
         rarity = 70000,
         coefficient = 400,
      },
      {
         id = "acid_slime",
         legacy_id = 194,
         item_type = 1,
         tags = { "slime" },
         level = 16,
         ai_calm = 1,
         ai_move = 60,
         ai_dist = 1,
         ai_act_sub_freq = 25,
         creaturepack = 3,
         normal_actions = { -1 },
         special_actions = { 455 },
         can_talk = 1,
         original_relationship = "Aggressive",
         race = "elona.slime",
         class_ = "elona.predator",
         element_of_unarmed_attack = 6300100,
         resistances = {
            -- Element acid is invalid in vanilla, although it is defined.
            -- ["elona.element_acid"] = 500,
         },
         image = 257,
         color = "Green",
         cspecialeq = 0,
         damage_reaction_info = 200063,
         fltselect = 0,
         category = 3,
         rarity = 70000,
         coefficient = 400,
      },
      {
         id = "bubble",
         legacy_id = 286,
         item_type = 1,
         tags = { "slime" },
         level = 9,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         original_relationship = "Aggressive",
         race = "elona.slime",
         resistances = {
            -- Element acid is invalid in vanilla, although it is defined.
            -- ["elona.element_acid"] = 500,
         },
         image = 400,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 25000,
         coefficient = 400,
         flags = { "Splits" },
      },
      {
         id = "blue_bubble",
         legacy_id = 285,
         item_type = 1,
         tags = { "slime" },
         level = 22,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         original_relationship = "Aggressive",
         race = "elona.slime",
         resistances = {
            -- Element acid is invalid in vanilla, although it is defined.
            -- ["elona.element_acid"] = 500,
         },
         image = 400,
         color = "LightBlue",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 25000,
         coefficient = 400,
         flags = { "Splits" },
      },
      {
         id = "mass_monster",
         legacy_id = 287,
         item_type = 1,
         tags = {},
         level = 20,
         ai_calm = 3,
         ai_move = 20,
         ai_dist = 1,
         ai_act_sub_freq = 15,
         creaturepack = 0,
         normal_actions = { -3 },
         special_actions = { 613 },
         original_relationship = "Aggressive",
         race = "elona.drake",
         image = 401,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 25000,
         coefficient = 400,
         flags = { "Splits" },
      },
      {
         id = "cube",
         legacy_id = 327,
         item_type = 1,
         tags = {},
         level = 52,
         ai_calm = 3,
         ai_move = 20,
         ai_dist = 1,
         ai_act_sub_freq = 20,
         creaturepack = 0,
         normal_actions = { -3 },
         special_actions = { 638 },
         original_relationship = "Aggressive",
         race = "elona.machine",
         image = 282,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 15000,
         coefficient = 400,
         flags = { "Splits2", "IsImmuneToElementalDamage" },
      },
      {
         id = "rabbit",
         legacy_id = 5,
         item_type = 1,
         tags = { "wild" },
         level = 1,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.rabbit",
         class_ = "elona.tourist",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         drops = { "elona.rabbit" },
      },
      {
         id = "snail",
         legacy_id = 6,
         item_type = 1,
         tags = { "wild" },
         level = 1,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         original_relationship = "Aggressive",
         race = "elona.snail",
         class_ = "elona.tourist",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "fallen_soldier",
         legacy_id = 7,
         item_type = 0,
         tags = { "man" },
         level = 3,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         original_relationship = "Unconcerned",
         race = "elona.norland",
         class_ = "elona.warrior",
         image = 37,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "mercenary",
         legacy_id = 8,
         item_type = 0,
         tags = { "man" },
         level = 4,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         original_relationship = "Unconcerned",
         race = "elona.norland",
         class_ = "elona.warrior",
         male_image = 139,
         female_image = 178,
         cspecialeq = 0,
         fltselect = 5,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "beggar",
         legacy_id = 9,
         item_type = 0,
         tags = { "man" },
         level = 2,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.norland",
         class_ = "elona.tourist",
         male_image = 102,
         female_image = 103,
         cspecialeq = 0,
         fltselect = 5,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "DropsGold" },
      },
      {
         id = "farmer",
         legacy_id = 269,
         item_type = 0,
         tags = { "man" },
         level = 5,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.norland",
         class_ = "elona.tourist",
         male_image = 179,
         female_image = 180,
         cspecialeq = 0,
         fltselect = 9,
         category = 0,
         rarity = 2000,
         coefficient = 400,
      },
      {
         id = "cleaner",
         legacy_id = 320,
         item_type = 0,
         tags = { "man" },
         level = 32,
         portrait = "__random__",
         ai_calm = 5,
         ai_move = 50,
         ai_dist = 3,
         ai_act_sub_freq = 30,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { -9996 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.gunner",
         sex = "Male",
         image = 410,
         cspecialeq = 0,
         fltselect = 9,
         category = 0,
         rarity = 10000,
         coefficient = 100,
      },
      {
         id = "miner",
         legacy_id = 273,
         item_type = 0,
         tags = { "man" },
         level = 5,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.norland",
         class_ = "elona.tourist",
         male_image = 187,
         female_image = 188,
         cspecialeq = 0,
         fltselect = 9,
         category = 0,
         rarity = 2000,
         coefficient = 400,
      },
      {
         id = "bard",
         legacy_id = 326,
         item_type = 0,
         tags = { "man" },
         level = 16,
         portrait = "__random__",
         ai_calm = 5,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.juere",
         class_ = "elona.pianist",
         image = 148,
         cspecialeq = 0,
         fltselect = 7,
         category = 0,
         rarity = 2000,
         coefficient = 400,
      },
      {
         id = "sister",
         legacy_id = 270,
         item_type = 0,
         tags = { "man" },
         level = 5,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 2,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.norland",
         class_ = "elona.tourist",
         male_image = 181,
         female_image = 182,
         cspecialeq = 0,
         fltselect = 9,
         category = 0,
         rarity = 2000,
         coefficient = 400,
      },
      {
         id = "holy_beast",
         legacy_id = 349,
         item_type = 0,
         tags = { "man", "god" },
         level = 12,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         original_relationship = "Unconcerned",
         race = "elona.norland",
         class_ = "elona.tourist",
         image = 442,
         cspecialeq = 0,
         fltselect = 1,
         category = 0,
         rarity = 2000,
         coefficient = 400,
         flags = { "IsQuickTempered" },
      },
      {
         id = "part_time_worker",
         legacy_id = 348,
         item_type = 0,
         tags = { "man" },
         level = 35,
         ai_calm = 3,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.pianist",
         image = 473,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 2000,
         coefficient = 400,
         flags = { "IsQuickTempered" },
         dialog_id = "elona.part_time_worker",
      },
      {
         id = "fanatic",
         legacy_id = 347,
         item_type = 0,
         tags = { "man" },
         level = 5,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         ai_heal = 404,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.norland",
         class_ = "elona.wizard",
         male_image = 285,
         female_image = 284,
         cspecialeq = 0,
         fltselect = 1,
         category = 0,
         rarity = 2000,
         coefficient = 400,
         flags = { "IsQuickTempered" },
      },
      {
         id = "rogue",
         legacy_id = 271,
         item_type = 0,
         tags = { "man" },
         level = 8,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 50,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 635 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.norland",
         class_ = "elona.thief",
         male_image = 183,
         female_image = 184,
         cspecialeq = 0,
         fltselect = 9,
         category = 0,
         rarity = 2000,
         coefficient = 400,
      },
      {
         id = "prostitute",
         legacy_id = 335,
         item_type = 0,
         tags = { "man" },
         level = 8,
         portrait = "__random__",
         ai_calm = 5,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.thief",
         male_image = 418,
         female_image = 417,
         cspecialeq = 0,
         fltselect = 9,
         category = 0,
         rarity = 2000,
         coefficient = 400,
      },
      {
         id = "prisoner",
         legacy_id = 337,
         item_type = 0,
         tags = { "man" },
         level = 3,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.norland",
         class_ = "elona.thief",
         male_image = 419,
         female_image = 420,
         cspecialeq = 0,
         fltselect = 9,
         category = 0,
         rarity = 2000,
         coefficient = 400,
      },
      {
         id = "artist",
         legacy_id = 272,
         item_type = 0,
         tags = { "man" },
         level = 6,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.norland",
         class_ = "elona.wizard",
         male_image = 185,
         female_image = 186,
         cspecialeq = 0,
         fltselect = 9,
         category = 0,
         rarity = 2000,
         coefficient = 400,
      },
      {
         id = "noble",
         legacy_id = 274,
         item_type = 0,
         tags = { "man" },
         level = 10,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.norland",
         class_ = "elona.warrior",
         male_image = 189,
         female_image = 190,
         cspecialeq = 0,
         fltselect = 9,
         category = 0,
         rarity = 2000,
         coefficient = 400,
      },
      {
         id = "mage_guild_member",
         legacy_id = 289,
         item_type = 3,
         tags = { "man" },
         level = 26,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 50,
         ai_dist = 2,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1, 414, 419, 422 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.eulderna",
         class_ = "elona.wizard",
         male_image = 195,
         female_image = 194,
         cspecialeq = 0,
         fltselect = 9,
         category = 0,
         rarity = 2000,
         coefficient = 400,
      },
      {
         id = "thief_guild_member",
         legacy_id = 293,
         item_type = 3,
         tags = { "man" },
         level = 26,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 50,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.juere",
         class_ = "elona.thief",
         male_image = 76,
         female_image = 77,
         cspecialeq = 0,
         fltselect = 9,
         category = 0,
         rarity = 2000,
         coefficient = 400,
      },
      {
         id = "fighter_guild_member",
         legacy_id = 295,
         item_type = 3,
         tags = { "man" },
         level = 26,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 90,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.norland",
         class_ = "elona.warrior",
         male_image = 78,
         female_image = 79,
         cspecialeq = 0,
         fltselect = 9,
         category = 0,
         rarity = 2000,
         coefficient = 400,
      },
      {
         id = "town_child",
         legacy_id = 35,
         item_type = 0,
         tags = { "man" },
         level = 1,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.norland",
         class_ = "elona.tourist",
         male_image = 104,
         female_image = 357,
         cspecialeq = 0,
         fltselect = 5,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "old_person",
         legacy_id = 36,
         item_type = 0,
         tags = { "man" },
         level = 1,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.norland",
         class_ = "elona.tourist",
         male_image = 106,
         female_image = 107,
         cspecialeq = 0,
         fltselect = 5,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "punk",
         legacy_id = 174,
         item_type = 0,
         tags = { "man", "sf" },
         level = 1,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 50,
         ai_dist = 2,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         original_relationship = "Unconcerned",
         race = "elona.norland",
         class_ = "elona.gunner",
         male_image = 112,
         female_image = 27,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 60000,
         coefficient = 400,
      },
      {
         id = "wild_sheep",
         legacy_id = 10,
         item_type = 1,
         tags = { "wild" },
         level = 1,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.sheep",
         class_ = "elona.tourist",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 30000,
         coefficient = 400,
      },
      {
         id = "flying_frog",
         legacy_id = 11,
         item_type = 1,
         tags = { "wild" },
         level = 2,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.frog",
         class_ = "elona.tourist",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsFloating" },
      },
      {
         id = "gangster",
         legacy_id = 12,
         item_type = 3,
         tags = { "man" },
         level = 3,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Aggressive",
         race = "elona.yerles",
         male_image = 31,
         female_image = 36,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "kobold",
         legacy_id = 13,
         item_type = 3,
         tags = {},
         level = 3,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 5,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.kobold",
         class_ = "elona.warrior",
         cspecialeq = 0,
         fltselect = 0,
         category = 5,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.poisonous",
      },
      {
         id = "yeek",
         legacy_id = 236,
         item_type = 3,
         tags = { "yeek" },
         level = 2,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 7,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.yeek",
         cspecialeq = 0,
         fltselect = 0,
         category = 7,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "yeek_warrior",
         legacy_id = 238,
         item_type = 3,
         tags = { "yeek" },
         level = 6,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 7,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.yeek",
         color = "Yellow",
         cspecialeq = 0,
         fltselect = 0,
         category = 7,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "yeek_archer",
         legacy_id = 241,
         item_type = 3,
         tags = { "yeek" },
         level = 4,
         ai_calm = 1,
         ai_move = 30,
         ai_dist = 2,
         ai_act_sub_freq = 0,
         creaturepack = 7,
         normal_actions = { -3 },
         original_relationship = "Aggressive",
         race = "elona.yeek",
         class_ = "elona.archer",
         color = "Green",
         cspecialeq = 0,
         fltselect = 0,
         category = 7,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "master_yeek",
         legacy_id = 240,
         item_type = 3,
         tags = { "yeek" },
         level = 9,
         ai_calm = 1,
         ai_move = 70,
         ai_dist = 2,
         ai_act_sub_freq = 20,
         creaturepack = 7,
         normal_actions = { -1, 410, 418, -3 },
         special_actions = { 640 },
         original_relationship = "Aggressive",
         race = "elona.yeek",
         color = "Purple",
         cspecialeq = 0,
         fltselect = 0,
         category = 7,
         rarity = 50000,
         coefficient = 400,
      },
      {
         id = "kamikaze_yeek",
         legacy_id = 237,
         item_type = 3,
         tags = { "yeek" },
         level = 6,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 12,
         normal_actions = { 644 },
         original_relationship = "Aggressive",
         race = "elona.yeek",
         color = "Red",
         cspecialeq = 0,
         fltselect = 0,
         category = 12,
         rarity = 150000,
         coefficient = 400,
         flags = { "Explodes" },
      },
      {
         id = "kamikaze_samurai",
         legacy_id = 244,
         item_type = 3,
         tags = {},
         level = 18,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 12,
         normal_actions = { 644 },
         can_talk = 3,
         original_relationship = "Aggressive",
         race = "elona.norland",
         class_ = "elona.warrior",
         image = 385,
         cspecialeq = 0,
         fltselect = 0,
         category = 12,
         rarity = 25000,
         coefficient = 400,
         flags = { "Explodes" },
      },
      {
         id = "bomb_rock",
         legacy_id = 245,
         item_type = 1,
         tags = {},
         level = 25,
         ai_calm = 1,
         ai_move = 10,
         ai_dist = 2,
         ai_act_sub_freq = 10,
         creaturepack = 0,
         normal_actions = { 644 },
         special_actions = { 410 },
         original_relationship = "Aggressive",
         race = "elona.rock",
         class_ = "elona.predator",
         image = 386,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 20000,
         coefficient = 400,
         flags = { "Explodes" },
      },
      {
         id = "hard_gay",
         legacy_id = 321,
         item_type = 3,
         tags = {},
         level = 10,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { 644 },
         can_talk = 4,
         original_relationship = "Aggressive",
         race = "elona.norland",
         class_ = "elona.predator",
         sex = "Male",
         image = 411,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 15000,
         coefficient = 200,
         flags = { "Explodes" },
      },
      {
         id = "rodlob",
         legacy_id = 242,
         item_type = 3,
         tags = { "yeek" },
         level = 14,
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 1,
         ai_act_sub_freq = 20,
         creaturepack = 7,
         normal_actions = { -1, -3, 640, 418 },
         special_actions = { 410, 645 },
         can_talk = 3,
         ai_heal = 402,
         original_relationship = "Aggressive",
         race = "elona.yeek",
         class_ = "elona.wizard",
         image = 381,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 7,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "hot_spring_maniac",
         legacy_id = 239,
         item_type = 3,
         tags = { "man" },
         level = 5,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.norland",
         male_image = 379,
         female_image = 380,
         cspecialeq = 0,
         fltselect = 7,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "centipede",
         legacy_id = 14,
         item_type = 2,
         tags = {},
         level = 4,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.centipede",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.poisonous",
      },
      {
         id = "mushroom",
         legacy_id = 15,
         item_type = 2,
         tags = {},
         level = 4,
         ai_calm = 3,
         ai_move = 0,
         ai_dist = 2,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -3, -3, 649 },
         original_relationship = "Aggressive",
         race = "elona.mushroom",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 50000,
         coefficient = 400,
      },
      {
         id = "spore_mushroom",
         legacy_id = 283,
         item_type = 2,
         tags = {},
         level = 8,
         ai_calm = 3,
         ai_move = 0,
         ai_dist = 2,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -3, 650 },
         original_relationship = "Aggressive",
         race = "elona.mushroom",
         element_of_unarmed_attack = 5400150,
         image = 399,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 50000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.poisonous",
      },
      {
         id = "chaos_mushroom",
         legacy_id = 284,
         item_type = 2,
         tags = {},
         level = 21,
         ai_calm = 3,
         ai_move = 0,
         ai_dist = 2,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -3, 650 },
         original_relationship = "Aggressive",
         race = "elona.mushroom",
         element_of_unarmed_attack = 5900250,
         image = 399,
         color = "Purple",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 50000,
         coefficient = 400,
      },
      {
         id = "citizen",
         legacy_id = 16,
         item_type = 3,
         tags = { "man" },
         level = 5,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Neutral",
         race = "elona.yerles",
         class_ = "elona.warrior",
         male_image = 136,
         female_image = 137,
         cspecialeq = 0,
         fltselect = 7,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "citizen2",
         legacy_id = 39,
         item_type = 3,
         tags = { "man" },
         level = 5,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Neutral",
         race = "elona.yerles",
         class_ = "elona.warrior",
         male_image = 134,
         female_image = 135,
         cspecialeq = 0,
         fltselect = 7,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "citizen_of_cyber_dome",
         legacy_id = 171,
         item_type = 3,
         tags = { "man" },
         level = 5,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Neutral",
         race = "elona.yerles",
         class_ = "elona.gunner",
         male_image = 128,
         female_image = 129,
         cspecialeq = 0,
         fltselect = 7,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "citizen_of_cyber_dome2",
         legacy_id = 172,
         item_type = 3,
         tags = { "man" },
         level = 5,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Neutral",
         race = "elona.yerles",
         class_ = "elona.gunner",
         male_image = 127,
         female_image = 131,
         cspecialeq = 0,
         fltselect = 7,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "sales_person",
         legacy_id = 173,
         item_type = 3,
         tags = { "man" },
         level = 20,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Neutral",
         race = "elona.yerles",
         class_ = "elona.gunner",
         male_image = 146,
         female_image = 147,
         cspecialeq = 0,
         fltselect = 7,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "sailor",
         legacy_id = 71,
         item_type = 3,
         tags = { "man" },
         level = 5,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         has_random_name = true,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.thief",
         male_image = 120,
         female_image = 121,
         cspecialeq = 0,
         fltselect = 7,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "captain",
         legacy_id = 72,
         item_type = 3,
         tags = { "man" },
         level = 5,
         portrait = "elona.man45",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         has_random_name = true,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.warrior",
         sex = "Male",
         image = 119,
         cspecialeq = 0,
         fltselect = 1,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "stersha",
         legacy_id = 79,
         item_type = 3,
         tags = { "man" },
         level = 25,
         portrait = "elona.stersha",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 4,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.warrior",
         sex = "Female",
         image = 117,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         dialog_id = "elona.stersha",
      },
      {
         id = "xabi",
         legacy_id = 80,
         item_type = 3,
         tags = { "man" },
         level = 35,
         portrait = "elona.xabi",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.warrior",
         sex = "Male",
         image = 116,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         dialog_id = "elona.xabi",
      },
      {
         id = "orc",
         legacy_id = 17,
         item_type = 3,
         tags = {},
         level = 5,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 2,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.orc",
         color = "LightBlue",
         cspecialeq = 0,
         fltselect = 0,
         category = 2,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "lizard_man",
         legacy_id = 281,
         item_type = 3,
         tags = { "dragon" },
         level = 7,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.lizardman",
         class_ = "elona.warrior",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsQuickTempered" },
      },
      {
         id = "minotaur",
         legacy_id = 282,
         item_type = 3,
         tags = { "mino" },
         level = 18,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.minotaur",
         class_ = "elona.warrior",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 70000,
         coefficient = 400,
      },
      {
         id = "minotaur_magician",
         legacy_id = 296,
         item_type = 3,
         tags = { "mino" },
         level = 22,
         ai_calm = 1,
         ai_move = 60,
         ai_dist = 1,
         ai_act_sub_freq = 10,
         creaturepack = 0,
         normal_actions = { -1, 420, 419, 415 },
         special_actions = { 645 },
         original_relationship = "Aggressive",
         race = "elona.minotaur",
         class_ = "elona.priest",
         color = "Blue",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 70000,
         coefficient = 400,
      },
      {
         id = "minotaur_boxer",
         legacy_id = 298,
         item_type = 3,
         tags = { "mino" },
         level = 23,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.minotaur",
         class_ = "elona.predator",
         color = "Red",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 70000,
         coefficient = 400,
         flags = { "IsQuickTempered" },
      },
      {
         id = "minotaur_king",
         legacy_id = 299,
         item_type = 3,
         tags = { "mino" },
         level = 25,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.minotaur",
         class_ = "elona.warrior",
         color = "Purple",
         cspecialeq = 1,
         eqtwohand = 1,
         fltselect = 0,
         category = 0,
         rarity = 40000,
         coefficient = 400,
      },
      {
         id = "ungaga",
         legacy_id = 300,
         item_type = 3,
         tags = { "mino" },
         level = 31,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 5,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 647 },
         original_relationship = "Aggressive",
         race = "elona.minotaur",
         class_ = "elona.warrior",
         color = "LightRed",
         fixlv = 6,
         cspecialeq = 1,
         eqweapon1 = 695,
         eqtwohand = 1,
         fltselect = 3,
         category = 0,
         rarity = 40000,
         coefficient = 400,
      },
      {
         id = "troll",
         legacy_id = 251,
         item_type = 3,
         tags = {},
         level = 14,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 2,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.troll",
         resistances = {
            ["elona.element_fire"] = 50,
         },
         cspecialeq = 0,
         fltselect = 0,
         category = 2,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.troll",
      },
      {
         id = "warrior_of_elea",
         legacy_id = 18,
         item_type = 3,
         tags = { "man" },
         level = 5,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 4,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.elea",
         class_ = "elona.warrior",
         female_image = 34,
         cspecialeq = 0,
         fltselect = 0,
         category = 4,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "wizard_of_elea",
         legacy_id = 24,
         item_type = 3,
         tags = { "man" },
         level = 5,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 20,
         creaturepack = 4,
         normal_actions = { -1, 414 },
         special_actions = { 416, 410 },
         original_relationship = "Aggressive",
         race = "elona.elea",
         class_ = "elona.wizard",
         male_image = 47,
         female_image = 42,
         cspecialeq = 0,
         fltselect = 0,
         category = 4,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "asura",
         legacy_id = 309,
         item_type = 3,
         tags = { "god" },
         level = 12,
         ai_calm = 1,
         ai_move = 90,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.asura",
         class_ = "elona.warrior",
         cspecialeq = 1,
         eqmultiweapon = 2,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "mitra",
         legacy_id = 310,
         item_type = 3,
         tags = { "god" },
         level = 26,
         ai_calm = 1,
         ai_move = 90,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.asura",
         class_ = "elona.warrior",
         color = "Green",
         cspecialeq = 1,
         eqmultiweapon = 266,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "varuna",
         legacy_id = 311,
         item_type = 3,
         tags = { "god" },
         level = 37,
         ai_calm = 1,
         ai_move = 90,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.asura",
         class_ = "elona.warrior",
         color = "Red",
         cspecialeq = 1,
         eqmultiweapon = 224,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "wizard",
         legacy_id = 41,
         item_type = 3,
         tags = { "man" },
         level = 5,
         portrait = "__random__",
         ai_calm = 2,
         ai_move = 100,
         ai_dist = 2,
         ai_act_sub_freq = 10,
         creaturepack = 0,
         normal_actions = { 414, 415 },
         special_actions = { 443, 447, 451 },
         has_random_name = true,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.wizard",
         male_image = 13,
         female_image = 14,
         cspecialeq = 0,
         fltselect = 7,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "warrior",
         legacy_id = 75,
         item_type = 3,
         tags = { "man" },
         level = 5,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         has_random_name = true,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.warrior",
         male_image = 21,
         female_image = 20,
         cspecialeq = 0,
         fltselect = 7,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "mandrake",
         legacy_id = 19,
         item_type = 2,
         tags = {},
         level = 5,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.mandrake",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.mandrake",
      },
      {
         id = "beetle",
         legacy_id = 22,
         item_type = 2,
         tags = {},
         level = 5,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.beetle",
         class_ = "elona.predator",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.beetle",
      },
      {
         id = "orc_warrior",
         legacy_id = 20,
         item_type = 3,
         tags = {},
         level = 10,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 2,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.orc",
         class_ = "elona.warrior",
         cspecialeq = 0,
         fltselect = 0,
         category = 2,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "goda",
         legacy_id = 25,
         item_type = 3,
         tags = {},
         level = 25,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 2,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.orc",
         class_ = "elona.warrior",
         color = "Red",
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 2,
         category = 2,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsQuickTempered" },
      },
      {
         id = "zombie",
         legacy_id = 21,
         item_type = 3,
         tags = { "undead" },
         level = 8,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 9,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.zombie",
         class_ = "elona.predator",
         cspecialeq = 0,
         fltselect = 0,
         category = 9,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.rotten_one",
         drops = { "elona.zombie" },
      },
      {
         id = "bat",
         legacy_id = 42,
         item_type = 1,
         tags = { "wild" },
         level = 1,
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 2,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.bat",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsFloating" },
      },
      {
         id = "vampire_bat",
         legacy_id = 43,
         item_type = 1,
         tags = { "wild" },
         level = 10,
         ai_calm = 1,
         ai_move = 60,
         ai_dist = 2,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1, 601 },
         original_relationship = "Aggressive",
         race = "elona.bat",
         class_ = "elona.predator",
         resistances = {
            ["elona.element_nether"] = 500,
         },
         color = "Red",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 70000,
         coefficient = 400,
         flags = { "IsFloating" },
      },
      {
         id = "dragon_bat",
         legacy_id = 44,
         item_type = 1,
         tags = { "wild", "fire", "dragon" },
         level = 30,
         ai_calm = 1,
         ai_move = 60,
         ai_dist = 2,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.bat",
         class_ = "elona.predator",
         color = "Blue",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 60000,
         coefficient = 400,
         flags = { "IsFloating" },
      },
      {
         id = "fire_ent",
         legacy_id = 45,
         item_type = 1,
         tags = { "fire" },
         level = 15,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.ent",
         class_ = "elona.predator",
         element_of_unarmed_attack = 5000200,
         resistances = {
            ["elona.element_cold"] = 50,
            ["elona.element_fire"] = 500,
         },
         color = "PaleRed",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.fire_ent",
      },
      {
         id = "ice_ent",
         legacy_id = 46,
         item_type = 1,
         tags = {},
         level = 15,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.ent",
         class_ = "elona.predator",
         element_of_unarmed_attack = 5100200,
         resistances = {
            ["elona.element_fire"] = 50,
            ["elona.element_cold"] = 500,
         },
         color = "LightBlue",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.ice_ent",
      },
      {
         id = "lich",
         legacy_id = 47,
         item_type = 6,
         tags = { "undead" },
         level = 20,
         ai_calm = 1,
         ai_move = 60,
         ai_dist = 1,
         ai_act_sub_freq = 30,
         creaturepack = 0,
         normal_actions = { -1, 414, 419, 422 },
         special_actions = { 410 },
         original_relationship = "Aggressive",
         race = "elona.lich",
         class_ = "elona.wizard",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 60000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.rotten_one",
      },
      {
         id = "master_lich",
         legacy_id = 48,
         item_type = 6,
         tags = { "undead" },
         level = 30,
         ai_calm = 1,
         ai_move = 60,
         ai_dist = 1,
         ai_act_sub_freq = 30,
         creaturepack = 0,
         normal_actions = { -1, 414, 419, 422, 450 },
         special_actions = { 410 },
         original_relationship = "Aggressive",
         race = "elona.lich",
         class_ = "elona.wizard",
         color = "PaleRed",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 50000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.rotten_one",
      },
      {
         id = "demi_lich",
         legacy_id = 49,
         item_type = 6,
         tags = { "undead" },
         level = 45,
         ai_calm = 1,
         ai_move = 60,
         ai_dist = 1,
         ai_act_sub_freq = 30,
         creaturepack = 0,
         normal_actions = { -1, 414, 419, 422 },
         special_actions = { 410 },
         original_relationship = "Aggressive",
         race = "elona.lich",
         class_ = "elona.wizard",
         color = "LightBlue",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 40000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.rotten_one",
      },
      {
         id = "executioner",
         legacy_id = 307,
         item_type = 6,
         tags = { "undead" },
         level = 18,
         ai_calm = 1,
         ai_move = 60,
         ai_dist = 1,
         ai_act_sub_freq = 20,
         creaturepack = 0,
         normal_actions = { -1, -1, 421, 410 },
         special_actions = { 646 },
         original_relationship = "Aggressive",
         race = "elona.lich",
         class_ = "elona.warrior",
         image = 404,
         cspecialeq = 1,
         eqweapon1 = 735,
         eqtwohand = 1,
         fltselect = 0,
         category = 0,
         rarity = 10000,
         coefficient = 400,
         flags = { "IsDeathMaster" },
         drops = { "elona.executioner" },
      },
      {
         id = "messenger_of_death",
         legacy_id = 308,
         item_type = 6,
         tags = { "undead" },
         level = 35,
         ai_calm = 1,
         ai_move = 70,
         ai_dist = 1,
         ai_act_sub_freq = 20,
         creaturepack = 0,
         normal_actions = { -1, 421, 410 },
         special_actions = { 646 },
         original_relationship = "Aggressive",
         race = "elona.lich",
         class_ = "elona.warrior",
         image = 404,
         color = "Blue",
         cspecialeq = 1,
         eqweapon1 = 735,
         fltselect = 0,
         category = 0,
         rarity = 10000,
         coefficient = 400,
         flags = { "IsDeathMaster" },
         drops = { "elona.executioner" },
      },
      {
         id = "hound",
         legacy_id = 50,
         item_type = 1,
         tags = { "wild" },
         level = 5,
         ai_calm = 1,
         ai_move = 30,
         ai_dist = 2,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.hound",
         class_ = "elona.predator",
         color = "Yellow",
         cspecialeq = 1,
         eqtwohand = 1,
         fltselect = 0,
         category = 0,
         rarity = 80000,
         coefficient = 400,
         flags = { "IsSuitableForMount" },
      },
      {
         id = "fire_hound",
         legacy_id = 51,
         item_type = 1,
         tags = { "wild", "fire" },
         level = 10,
         ai_calm = 1,
         ai_move = 30,
         ai_dist = 2,
         ai_act_sub_freq = 12,
         creaturepack = 14,
         normal_actions = { -1 },
         special_actions = { 602 },
         original_relationship = "Aggressive",
         race = "elona.hound",
         class_ = "elona.predator",
         resistances = {
            ["elona.element_cold"] = 50,
            ["elona.element_fire"] = 500,
         },
         color = "Red",
         cspecialeq = 0,
         fltselect = 0,
         category = 14,
         rarity = 70000,
         coefficient = 400,
         flags = { "IsSuitableForMount" },
      },
      {
         id = "ice_hound",
         legacy_id = 52,
         item_type = 1,
         tags = { "wild" },
         level = 10,
         ai_calm = 1,
         ai_move = 30,
         ai_dist = 2,
         ai_act_sub_freq = 12,
         creaturepack = 15,
         normal_actions = { -1 },
         special_actions = { 603 },
         original_relationship = "Aggressive",
         race = "elona.hound",
         class_ = "elona.predator",
         resistances = {
            ["elona.element_fire"] = 50,
            ["elona.element_cold"] = 500,
         },
         color = "White2",
         cspecialeq = 0,
         fltselect = 0,
         category = 15,
         rarity = 70000,
         coefficient = 400,
         flags = { "IsSuitableForMount" },
      },
      {
         id = "lightning_hound",
         legacy_id = 53,
         item_type = 1,
         tags = { "wild" },
         level = 12,
         ai_calm = 1,
         ai_move = 30,
         ai_dist = 2,
         ai_act_sub_freq = 12,
         creaturepack = 16,
         normal_actions = { -1 },
         special_actions = { 604 },
         original_relationship = "Aggressive",
         race = "elona.hound",
         class_ = "elona.predator",
         resistances = {
            ["elona.element_lightning"] = 500,
         },
         color = "Orange",
         cspecialeq = 0,
         fltselect = 0,
         category = 16,
         rarity = 70000,
         coefficient = 400,
         flags = { "IsSuitableForMount" },
      },
      {
         id = "dark_hound",
         legacy_id = 54,
         item_type = 1,
         tags = { "wild" },
         level = 12,
         ai_calm = 1,
         ai_move = 30,
         ai_dist = 2,
         ai_act_sub_freq = 12,
         creaturepack = 17,
         normal_actions = { -1 },
         special_actions = { 605 },
         original_relationship = "Aggressive",
         race = "elona.hound",
         class_ = "elona.predator",
         resistances = {
            ["elona.element_darkness"] = 500,
         },
         color = "Blue",
         cspecialeq = 0,
         fltselect = 0,
         category = 17,
         rarity = 70000,
         coefficient = 400,
         flags = { "IsSuitableForMount" },
      },
      {
         id = "illusion_hound",
         legacy_id = 55,
         item_type = 1,
         tags = { "wild" },
         level = 18,
         ai_calm = 1,
         ai_move = 30,
         ai_dist = 2,
         ai_act_sub_freq = 12,
         creaturepack = 18,
         normal_actions = { -1 },
         special_actions = { 611 },
         original_relationship = "Aggressive",
         race = "elona.hound",
         class_ = "elona.predator",
         resistances = {
            ["elona.element_mind"] = 500,
         },
         color = "LightRed",
         cspecialeq = 0,
         fltselect = 0,
         category = 18,
         rarity = 50000,
         coefficient = 400,
         flags = { "IsSuitableForMount" },
      },
      {
         id = "nerve_hound",
         legacy_id = 56,
         item_type = 1,
         tags = { "wild" },
         level = 18,
         ai_calm = 1,
         ai_move = 30,
         ai_dist = 2,
         ai_act_sub_freq = 12,
         creaturepack = 19,
         normal_actions = { -1 },
         special_actions = { 609 },
         original_relationship = "Aggressive",
         race = "elona.hound",
         class_ = "elona.predator",
         resistances = {
            ["elona.element_nerve"] = 500,
         },
         color = "Cyan",
         cspecialeq = 0,
         fltselect = 0,
         category = 19,
         rarity = 50000,
         coefficient = 400,
         flags = { "IsSuitableForMount" },
      },
      {
         id = "poison_hound",
         legacy_id = 57,
         item_type = 1,
         tags = { "wild" },
         level = 15,
         ai_calm = 1,
         ai_move = 30,
         ai_dist = 2,
         ai_act_sub_freq = 12,
         creaturepack = 20,
         normal_actions = { -1 },
         special_actions = { 610 },
         original_relationship = "Aggressive",
         race = "elona.hound",
         class_ = "elona.predator",
         resistances = {
            ["elona.element_poison"] = 500,
         },
         color = "Green",
         cspecialeq = 0,
         fltselect = 0,
         category = 20,
         rarity = 50000,
         coefficient = 400,
         flags = { "IsSuitableForMount" },
      },
      {
         id = "sound_hound",
         legacy_id = 58,
         item_type = 1,
         tags = { "wild" },
         level = 22,
         ai_calm = 1,
         ai_move = 30,
         ai_dist = 2,
         ai_act_sub_freq = 12,
         creaturepack = 21,
         normal_actions = { -1 },
         special_actions = { 607 },
         original_relationship = "Aggressive",
         race = "elona.hound",
         class_ = "elona.predator",
         resistances = {
            ["elona.element_sound"] = 500,
         },
         color = "Gold",
         cspecialeq = 0,
         fltselect = 0,
         category = 21,
         rarity = 40000,
         coefficient = 400,
         flags = { "IsSuitableForMount" },
      },
      {
         id = "nether_hound",
         legacy_id = 59,
         item_type = 1,
         tags = { "wild" },
         level = 25,
         ai_calm = 1,
         ai_move = 30,
         ai_dist = 2,
         ai_act_sub_freq = 12,
         creaturepack = 22,
         normal_actions = { -1 },
         special_actions = { 608 },
         original_relationship = "Aggressive",
         race = "elona.hound",
         class_ = "elona.predator",
         resistances = {
            ["elona.element_nether"] = 500,
         },
         color = "LightGrey",
         cspecialeq = 0,
         fltselect = 0,
         category = 22,
         rarity = 40000,
         coefficient = 400,
         flags = { "IsSuitableForMount" },
      },
      {
         id = "chaos_hound",
         legacy_id = 60,
         item_type = 1,
         tags = { "wild" },
         level = 30,
         ai_calm = 1,
         ai_move = 30,
         ai_dist = 2,
         ai_act_sub_freq = 12,
         creaturepack = 23,
         normal_actions = { -1 },
         special_actions = { 606 },
         original_relationship = "Aggressive",
         race = "elona.hound",
         class_ = "elona.predator",
         resistances = {
            ["elona.element_chaos"] = 500,
         },
         color = "LightPurple",
         cspecialeq = 0,
         fltselect = 0,
         category = 23,
         rarity = 40000,
         coefficient = 400,
         flags = { "IsSuitableForMount" },
      },
      {
         id = "giant_squirrel",
         legacy_id = 61,
         item_type = 1,
         tags = { "wild" },
         level = 4,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.rabbit",
         class_ = "elona.predator",
         image = 204,
         color = "Yellow",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.calm",
      },
      {
         id = "killer_squirrel",
         legacy_id = 62,
         item_type = 1,
         tags = { "wild" },
         level = 10,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.rabbit",
         class_ = "elona.predator",
         image = 204,
         color = "Red",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.calm",
      },
      {
         id = "grudge",
         legacy_id = 63,
         item_type = 3,
         tags = { "undead" },
         level = 7,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 10,
         creaturepack = 0,
         normal_actions = { -1, 613 },
         special_actions = { 447, 449 },
         original_relationship = "Aggressive",
         race = "elona.ghost",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsFloating" },
         corpse_eating_callback = "elona.eating_effect.grudge",
      },
      {
         id = "hungry_demon",
         legacy_id = 64,
         item_type = 3,
         tags = { "undead" },
         level = 3,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1, 614 },
         original_relationship = "Aggressive",
         race = "elona.ghost",
         color = "Green",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 70000,
         coefficient = 400,
         flags = { "IsFloating" },
         corpse_eating_callback = "elona.eating_effect.grudge",
      },
      {
         id = "hungry_sea_lion",
         legacy_id = 312,
         item_type = 3,
         tags = {},
         level = 8,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 25,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 651 },
         original_relationship = "Aggressive",
         race = "elona.ent",
         class_ = "elona.predator",
         image = 406,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 40000,
         coefficient = 400,
         flags = { "IsFloating" },
      },
      {
         id = "super_hungry_sea_lion",
         legacy_id = 313,
         item_type = 3,
         tags = {},
         level = 19,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 25,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 651 },
         original_relationship = "Aggressive",
         race = "elona.ent",
         class_ = "elona.predator",
         image = 406,
         color = "PaleRed",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 40000,
         coefficient = 400,
         flags = { "IsFloating" },
      },
      {
         id = "electric_cloud",
         legacy_id = 65,
         item_type = 1,
         tags = {},
         level = 12,
         ai_calm = 1,
         ai_move = 70,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1, 421, 604 },
         original_relationship = "Aggressive",
         race = "elona.spirit",
         resistances = {
            ["elona.element_lightning"] = 500,
         },
         color = "Orange",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsFloating", "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.electric_cloud",
      },
      {
         id = "chaos_cloud",
         legacy_id = 66,
         item_type = 1,
         tags = {},
         level = 30,
         ai_calm = 1,
         ai_move = 70,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1, 433 },
         original_relationship = "Aggressive",
         race = "elona.spirit",
         element_of_unarmed_attack = 5900300,
         resistances = {
            ["elona.element_chaos"] = 500,
         },
         color = "LightPurple",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsFloating", "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.chaos_cloud",
      },
      {
         id = "floating_eye",
         legacy_id = 67,
         item_type = 1,
         tags = {},
         level = 2,
         ai_calm = 3,
         ai_move = 15,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -3 },
         original_relationship = "Aggressive",
         race = "elona.eye",
         element_of_unarmed_attack = 5800250,
         resistances = {
            ["elona.element_mind"] = 500,
            ["elona.element_nerve"] = 500,
         },
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 80000,
         coefficient = 400,
         flags = { "IsFloating", "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.floating_eye",
      },
      {
         id = "chaos_eye",
         legacy_id = 315,
         item_type = 1,
         tags = {},
         level = 14,
         ai_calm = 3,
         ai_move = 15,
         ai_dist = 1,
         ai_act_sub_freq = 10,
         creaturepack = 0,
         normal_actions = { -3 },
         special_actions = { 632 },
         original_relationship = "Aggressive",
         race = "elona.eye",
         class_ = "elona.predator",
         element_of_unarmed_attack = 5900400,
         resistances = {
            ["elona.element_mind"] = 500,
            ["elona.element_nerve"] = 500,
         },
         color = "Purple",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 60000,
         coefficient = 400,
         flags = { "IsFloating", "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.chaos_eye",
      },
      {
         id = "mad_gaze",
         legacy_id = 316,
         item_type = 1,
         tags = {},
         level = 7,
         ai_calm = 3,
         ai_move = 15,
         ai_dist = 1,
         ai_act_sub_freq = 30,
         creaturepack = 0,
         normal_actions = { -3 },
         special_actions = { 636 },
         original_relationship = "Aggressive",
         race = "elona.eye",
         element_of_unarmed_attack = 5400300,
         resistances = {
            ["elona.element_mind"] = 500,
            ["elona.element_nerve"] = 500,
         },
         image = 407,
         color = "Blue",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 60000,
         coefficient = 400,
         flags = { "IsFloating", "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.mad_gaze",
      },
      {
         id = "death_gaze",
         legacy_id = 314,
         item_type = 1,
         tags = {},
         level = 29,
         ai_calm = 3,
         ai_move = 15,
         ai_dist = 1,
         ai_act_sub_freq = 20,
         creaturepack = 0,
         normal_actions = { -3 },
         special_actions = { 652 },
         original_relationship = "Aggressive",
         race = "elona.eye",
         class_ = "elona.predator",
         element_of_unarmed_attack = 5800450,
         resistances = {
            ["elona.element_mind"] = 500,
            ["elona.element_nerve"] = 500,
         },
         image = 407,
         color = "Red",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 60000,
         coefficient = 400,
         flags = { "IsFloating", "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.floating_eye",
      },
      {
         id = "wyvern",
         legacy_id = 68,
         item_type = 4,
         tags = { "dragon" },
         level = 20,
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 1,
         ai_act_sub_freq = 15,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 602 },
         original_relationship = "Aggressive",
         race = "elona.wyvern",
         class_ = "elona.predator",
         resistances = {
            ["elona.element_fire"] = 500,
         },
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 50000,
         coefficient = 400,
         flags = { "IsFloating" },
      },
      {
         id = "puppet",
         legacy_id = 78,
         item_type = 3,
         tags = {},
         level = 15,
         ai_calm = 1,
         ai_move = 40,
         ai_dist = 3,
         ai_act_sub_freq = 20,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 449, 447, 450 },
         original_relationship = "Aggressive",
         race = "elona.eulderna",
         class_ = "elona.predator",
         image = 209,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "wasp",
         legacy_id = 81,
         item_type = 2,
         tags = {},
         level = 5,
         ai_calm = 1,
         ai_move = 30,
         ai_dist = 2,
         ai_act_sub_freq = 30,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 615 },
         original_relationship = "Aggressive",
         race = "elona.wasp",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsFloating" },
      },
      {
         id = "red_wasp",
         legacy_id = 82,
         item_type = 2,
         tags = { "fire" },
         level = 10,
         ai_calm = 1,
         ai_move = 30,
         ai_dist = 2,
         ai_act_sub_freq = 30,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 616 },
         original_relationship = "Aggressive",
         race = "elona.wasp",
         color = "Red",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsFloating" },
      },
      {
         id = "cyclops",
         legacy_id = 83,
         item_type = 3,
         tags = {},
         level = 22,
         ai_calm = 1,
         ai_move = 85,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.giant",
         class_ = "elona.warrior",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 60000,
         coefficient = 400,
         flags = { "IsQuickTempered" },
         corpse_eating_callback = "elona.eating_effect.cyclops",
      },
      {
         id = "titan",
         legacy_id = 84,
         item_type = 3,
         tags = {},
         level = 40,
         ai_calm = 1,
         ai_move = 85,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         original_relationship = "Aggressive",
         race = "elona.giant",
         class_ = "elona.warrior",
         color = "PaleRed",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 50000,
         coefficient = 400,
         flags = { "IsQuickTempered" },
         corpse_eating_callback = "elona.eating_effect.titan",
      },
      {
         id = "imp",
         legacy_id = 85,
         item_type = 3,
         tags = { "fire" },
         level = 7,
         ai_calm = 1,
         ai_move = 50,
         ai_dist = 2,
         ai_act_sub_freq = 15,
         creaturepack = 0,
         normal_actions = { -1, -1, 414 },
         special_actions = { 410, 450 },
         original_relationship = "Aggressive",
         race = "elona.imp",
         color = "PaleRed",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsFloating" },
         corpse_eating_callback = "elona.eating_effect.imp",
         drops = { "elona.imp" },
      },
      {
         id = "nether_imp",
         legacy_id = 86,
         item_type = 3,
         tags = { "god" },
         level = 16,
         ai_calm = 1,
         ai_move = 50,
         ai_dist = 2,
         ai_act_sub_freq = 15,
         creaturepack = 0,
         normal_actions = { -1, 415, 414 },
         special_actions = { 410 },
         original_relationship = "Aggressive",
         race = "elona.imp",
         color = "Blue",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsFloating" },
         corpse_eating_callback = "elona.eating_effect.imp",
         drops = { "elona.imp" },
      },
      {
         id = "chaos_imp",
         legacy_id = 87,
         item_type = 3,
         tags = {},
         level = 27,
         ai_calm = 1,
         ai_move = 50,
         ai_dist = 2,
         ai_act_sub_freq = 15,
         creaturepack = 0,
         normal_actions = { -1, 417, 414 },
         special_actions = { 410 },
         original_relationship = "Aggressive",
         race = "elona.imp",
         color = "LightPurple",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsFloating" },
         corpse_eating_callback = "elona.eating_effect.imp",
         drops = { "elona.imp" },
      },
      {
         id = "hand_of_the_dead",
         legacy_id = 88,
         item_type = 3,
         tags = { "undead" },
         level = 4,
         ai_calm = 3,
         ai_move = 25,
         ai_dist = 3,
         ai_act_sub_freq = 20,
         creaturepack = 0,
         normal_actions = { -3 },
         special_actions = { 620, 613 },
         original_relationship = "Aggressive",
         race = "elona.hand",
         element_of_unarmed_attack = 5300080,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.hand",
      },
      {
         id = "hand_of_the_chaos",
         legacy_id = 89,
         item_type = 3,
         tags = { "undead" },
         level = 11,
         ai_calm = 3,
         ai_move = 25,
         ai_dist = 3,
         ai_act_sub_freq = 20,
         creaturepack = 0,
         normal_actions = { -3 },
         special_actions = { 620 },
         original_relationship = "Aggressive",
         race = "elona.hand",
         element_of_unarmed_attack = 5900180,
         color = "LightPurple",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.hand",
      },
      {
         id = "hand_of_the_murderer",
         legacy_id = 90,
         item_type = 3,
         tags = { "undead" },
         level = 15,
         ai_calm = 3,
         ai_move = 25,
         ai_dist = 3,
         ai_act_sub_freq = 20,
         creaturepack = 0,
         normal_actions = { -3 },
         special_actions = { 620, 449 },
         original_relationship = "Aggressive",
         race = "elona.hand",
         class_ = "elona.warrior",
         color = "PaleRed",
         cspecialeq = 1,
         eqtwohand = 1,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.hand",
      },
      {
         id = "ghost",
         legacy_id = 91,
         item_type = 3,
         tags = { "undead" },
         level = 5,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1, 617, 613 },
         original_relationship = "Aggressive",
         race = "elona.ghost",
         element_of_unarmed_attack = 5600080,
         image = 214,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsFloating" },
         corpse_eating_callback = "elona.eating_effect.ghost",
      },
      {
         id = "nymph",
         legacy_id = 92,
         item_type = 3,
         tags = { "undead" },
         level = 13,
         ai_calm = 1,
         ai_move = 60,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { 618, 618, 419, 603 },
         original_relationship = "Aggressive",
         race = "elona.ghost",
         element_of_unarmed_attack = 5400200,
         resistances = {
            ["elona.element_cold"] = 500,
         },
         image = 214,
         color = "Orange",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsFloating" },
         corpse_eating_callback = "elona.eating_effect.nymph",
      },
      {
         id = "man_eater_flower",
         legacy_id = 93,
         item_type = 2,
         tags = {},
         level = 8,
         ai_calm = 3,
         ai_move = 20,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -3 },
         original_relationship = "Aggressive",
         race = "elona.mandrake",
         element_of_unarmed_attack = 5400200,
         image = 215,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "chaos_flower",
         legacy_id = 94,
         item_type = 2,
         tags = {},
         level = 19,
         ai_calm = 3,
         ai_move = 20,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -3 },
         original_relationship = "Aggressive",
         race = "elona.mandrake",
         element_of_unarmed_attack = 5900250,
         image = 215,
         color = "LightRed",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "cobra",
         legacy_id = 95,
         item_type = 1,
         tags = { "wild" },
         level = 10,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 30,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 615 },
         original_relationship = "Aggressive",
         race = "elona.snake",
         color = "LightBlue",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.cobra",
      },
      {
         id = "king_cobra",
         legacy_id = 96,
         item_type = 1,
         tags = { "wild" },
         level = 18,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 30,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 615 },
         original_relationship = "Aggressive",
         race = "elona.snake",
         color = "PaleRed",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.cobra",
      },
      {
         id = "fire_drake",
         legacy_id = 97,
         item_type = 4,
         tags = { "fire", "dragon" },
         level = 16,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 20,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 602 },
         original_relationship = "Aggressive",
         race = "elona.drake",
         resistances = {
            ["elona.element_fire"] = 500,
         },
         color = "Red",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "ice_drake",
         legacy_id = 98,
         item_type = 4,
         tags = { "dragon" },
         level = 16,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 20,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 603 },
         original_relationship = "Aggressive",
         race = "elona.drake",
         resistances = {
            ["elona.element_cold"] = 500,
         },
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "lesser_mummy",
         legacy_id = 99,
         item_type = 3,
         tags = { "undead" },
         level = 7,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 20,
         creaturepack = 13,
         normal_actions = { -1 },
         special_actions = { 617, 613 },
         original_relationship = "Aggressive",
         race = "elona.zombie",
         image = 219,
         cspecialeq = 0,
         fltselect = 0,
         category = 13,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.rotten_one",
         drops = { "elona.mummy" },
      },
      {
         id = "mummy",
         legacy_id = 100,
         item_type = 3,
         tags = { "undead" },
         level = 14,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 20,
         creaturepack = 13,
         normal_actions = { -1 },
         special_actions = { 617, 613 },
         original_relationship = "Aggressive",
         race = "elona.zombie",
         class_ = "elona.warrior",
         image = 219,
         color = "LightRed",
         cspecialeq = 0,
         fltselect = 0,
         category = 13,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.rotten_one",
      },
      {
         id = "greater_mummy",
         legacy_id = 101,
         item_type = 3,
         tags = { "undead" },
         level = 22,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 20,
         creaturepack = 13,
         normal_actions = { -1 },
         special_actions = { 617, 613 },
         original_relationship = "Aggressive",
         race = "elona.zombie",
         class_ = "elona.warrior",
         image = 219,
         color = "Orange",
         cspecialeq = 0,
         fltselect = 0,
         category = 13,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.rotten_one",
      },
      {
         id = "tuwen",
         legacy_id = 257,
         item_type = 3,
         tags = { "undead" },
         level = 28,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 30,
         creaturepack = 13,
         normal_actions = { -1 },
         special_actions = { 646, 613 },
         original_relationship = "Aggressive",
         race = "elona.zombie",
         class_ = "elona.warrior",
         image = 219,
         color = "Purple",
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 13,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsDeathMaster" },
         corpse_eating_callback = "elona.eating_effect.rotten_one",
         drops = { "elona.tuwen" }
      },
      {
         id = "ancient_coffin",
         legacy_id = 254,
         item_type = 3,
         tags = { "undead" },
         level = 19,
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 1,
         ai_act_sub_freq = 15,
         creaturepack = 13,
         normal_actions = { -1, -1, 638, 449 },
         special_actions = { 645 },
         original_relationship = "Aggressive",
         race = "elona.zombie",
         image = 395,
         cspecialeq = 0,
         fltselect = 0,
         category = 13,
         rarity = 50000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.rotten_one",
      },
      {
         id = "goblin",
         legacy_id = 102,
         item_type = 3,
         tags = {},
         level = 2,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 1,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.goblin",
         color = "PaleRed",
         cspecialeq = 0,
         fltselect = 0,
         category = 1,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "goblin_warrior",
         legacy_id = 103,
         item_type = 3,
         tags = {},
         level = 6,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 1,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.goblin",
         class_ = "elona.warrior",
         color = "Orange",
         cspecialeq = 0,
         fltselect = 0,
         category = 1,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "goblin_shaman",
         legacy_id = 104,
         item_type = 3,
         tags = {},
         level = 8,
         ai_calm = 1,
         ai_move = 85,
         ai_dist = 1,
         ai_act_sub_freq = 15,
         creaturepack = 1,
         normal_actions = { -1, 420 },
         special_actions = { 425, 447 },
         ai_heal = 400,
         original_relationship = "Aggressive",
         race = "elona.goblin",
         class_ = "elona.warmage",
         color = "LightPurple",
         cspecialeq = 0,
         fltselect = 0,
         category = 1,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "goblin_wizard",
         legacy_id = 105,
         item_type = 3,
         tags = {},
         level = 10,
         ai_calm = 1,
         ai_move = 60,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 1,
         normal_actions = { -1, 414 },
         original_relationship = "Aggressive",
         race = "elona.goblin",
         class_ = "elona.wizard",
         color = "Blue",
         cspecialeq = 0,
         fltselect = 0,
         category = 1,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "red_baptist",
         legacy_id = 106,
         item_type = 3,
         tags = { "undead", "fire" },
         level = 12,
         ai_calm = 1,
         ai_move = 50,
         ai_dist = 2,
         ai_act_sub_freq = 25,
         creaturepack = 0,
         normal_actions = { -1, 420 },
         special_actions = { 410 },
         original_relationship = "Aggressive",
         race = "elona.ghost",
         class_ = "elona.wizard",
         resistances = {
            ["elona.element_fire"] = 500,
         },
         image = 221,
         color = "Red",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsImmuneToFear" },
      },
      {
         id = "blue_baptist",
         legacy_id = 107,
         item_type = 3,
         tags = { "undead" },
         level = 12,
         ai_calm = 1,
         ai_move = 50,
         ai_dist = 2,
         ai_act_sub_freq = 25,
         creaturepack = 0,
         normal_actions = { -1, 419 },
         special_actions = { 410 },
         original_relationship = "Aggressive",
         race = "elona.ghost",
         class_ = "elona.wizard",
         resistances = {
            ["elona.element_cold"] = 500,
         },
         image = 221,
         color = "Blue",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsImmuneToFear" },
      },
      {
         id = "brown_bear",
         legacy_id = 108,
         item_type = 1,
         tags = { "wild" },
         level = 4,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 11,
         normal_actions = { -1 },
         can_talk = 2,
         has_random_name = true,
         original_relationship = "Aggressive",
         race = "elona.bear",
         cspecialeq = 0,
         fltselect = 0,
         category = 11,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "grizzly",
         legacy_id = 109,
         item_type = 1,
         tags = { "wild" },
         level = 10,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 11,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.bear",
         color = "Red",
         cspecialeq = 0,
         fltselect = 0,
         category = 11,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsQuickTempered" },
      },
      {
         id = "mammoth",
         legacy_id = 344,
         item_type = 1,
         tags = { "wild" },
         level = 28,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 11,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.bear",
         image = 439,
         cspecialeq = 0,
         fltselect = 0,
         category = 11,
         rarity = 50000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.mammoth",
      },
      {
         id = "living_armor",
         legacy_id = 110,
         item_type = 3,
         tags = { "undead" },
         level = 15,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.armor",
         class_ = "elona.warrior",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 40000,
         coefficient = 400,
         flags = { "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.iron",
      },
      {
         id = "steel_mass",
         legacy_id = 111,
         item_type = 3,
         tags = { "undead" },
         level = 25,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.armor",
         class_ = "elona.warrior",
         color = "LightBlue",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 30000,
         coefficient = 400,
         flags = { "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.iron",
      },
      {
         id = "golden_armor",
         legacy_id = 112,
         item_type = 3,
         tags = { "undead" },
         level = 35,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.armor",
         class_ = "elona.warrior",
         color = "Orange",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 30000,
         coefficient = 400,
         flags = { "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.iron",
      },
      {
         id = "death_armor",
         legacy_id = 113,
         item_type = 3,
         tags = {},
         level = 45,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 10,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 613 },
         original_relationship = "Aggressive",
         race = "elona.armor",
         class_ = "elona.warrior",
         color = "PaleRed",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 30000,
         coefficient = 400,
         flags = { "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.iron",
      },
      {
         id = "medusa",
         legacy_id = 114,
         item_type = 3,
         tags = { "god" },
         level = 22,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.medusa",
         class_ = "elona.warmage",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "euryale",
         legacy_id = 115,
         item_type = 3,
         tags = { "god" },
         level = 33,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.medusa",
         class_ = "elona.warmage",
         color = "Orange",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "stheno",
         legacy_id = 116,
         item_type = 3,
         tags = { "god" },
         level = 44,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.medusa",
         class_ = "elona.warmage",
         color = "PaleRed",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "cupid_of_love",
         legacy_id = 117,
         item_type = 3,
         tags = { "god" },
         level = 8,
         ai_calm = 1,
         ai_move = 50,
         ai_dist = 2,
         ai_act_sub_freq = 20,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 443 },
         original_relationship = "Aggressive",
         race = "elona.cupid",
         class_ = "elona.archer",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsFloating" },
         corpse_eating_callback = "elona.eating_effect.cupid_of_love",
         drops = { "elona.cupid_of_love" },
      },
      {
         id = "lesser_phantom",
         legacy_id = 118,
         item_type = 3,
         tags = { "undead" },
         level = 9,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 20,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 447 },
         original_relationship = "Aggressive",
         race = "elona.phantom",
         class_ = "elona.wizard",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsFloating", "IsImmuneToFear" },
      },
      {
         id = "tyrannosaurus",
         legacy_id = 248,
         item_type = 1,
         tags = { "dragon" },
         level = 30,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.dinosaur",
         class_ = "elona.predator",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 50000,
         coefficient = 400,
      },
      {
         id = "harpy",
         legacy_id = 119,
         item_type = 3,
         tags = {},
         level = 13,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.harpy",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "green_dragon",
         legacy_id = 120,
         item_type = 5,
         tags = { "dragon" },
         level = 32,
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 1,
         ai_act_sub_freq = 15,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 612 },
         original_relationship = "Aggressive",
         race = "elona.dragon",
         class_ = "elona.predator",
         color = "LightGreen",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 30000,
         coefficient = 400,
      },
      {
         id = "red_dragon",
         legacy_id = 121,
         item_type = 5,
         tags = { "fire", "dragon" },
         level = 40,
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 1,
         ai_act_sub_freq = 15,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 602 },
         original_relationship = "Aggressive",
         race = "elona.dragon",
         class_ = "elona.predator",
         resistances = {
            ["elona.element_fire"] = 500,
         },
         color = "Red",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 20000,
         coefficient = 400,
      },
      {
         id = "white_dragon",
         legacy_id = 122,
         item_type = 5,
         tags = { "dragon" },
         level = 40,
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 1,
         ai_act_sub_freq = 15,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 603 },
         original_relationship = "Aggressive",
         race = "elona.dragon",
         class_ = "elona.predator",
         resistances = {
            ["elona.element_cold"] = 500,
         },
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 20000,
         coefficient = 400,
      },
      {
         id = "elec_dragon",
         legacy_id = 123,
         item_type = 5,
         tags = { "dragon" },
         level = 40,
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 1,
         ai_act_sub_freq = 15,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 604 },
         original_relationship = "Aggressive",
         race = "elona.dragon",
         class_ = "elona.predator",
         resistances = {
            ["elona.element_lightning"] = 500,
         },
         color = "Orange",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 20000,
         coefficient = 400,
      },
      {
         id = "nether_dragon",
         legacy_id = 124,
         item_type = 5,
         tags = { "undead", "dragon" },
         level = 45,
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 1,
         ai_act_sub_freq = 15,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 608 },
         original_relationship = "Aggressive",
         race = "elona.dragon",
         class_ = "elona.predator",
         resistances = {
            ["elona.element_nether"] = 500,
         },
         color = "Blue",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 10000,
         coefficient = 400,
      },
      {
         id = "chaos_dragon",
         legacy_id = 125,
         item_type = 5,
         tags = { "dragon" },
         level = 50,
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 1,
         ai_act_sub_freq = 15,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 606 },
         original_relationship = "Aggressive",
         race = "elona.dragon",
         class_ = "elona.predator",
         resistances = {
            ["elona.element_chaos"] = 500,
         },
         color = "LightPurple",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 10000,
         coefficient = 400,
      },
      {
         id = "cerberus",
         legacy_id = 126,
         item_type = 4,
         tags = { "fire" },
         level = 23,
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 1,
         ai_act_sub_freq = 15,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 602 },
         original_relationship = "Aggressive",
         race = "elona.cerberus",
         class_ = "elona.predator",
         resistances = {
            ["elona.element_fire"] = 500,
         },
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 40000,
         coefficient = 400,
      },
      {
         id = "scorpion",
         legacy_id = 255,
         item_type = 2,
         tags = { "wild" },
         level = 4,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.centipede",
         element_of_unarmed_attack = 5500150,
         resistances = {
            ["elona.element_poison"] = 500,
         },
         image = 396,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.cobra",
      },
      {
         id = "king_scorpion",
         legacy_id = 256,
         item_type = 2,
         tags = { "wild" },
         level = 24,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.centipede",
         element_of_unarmed_attack = 5500350,
         resistances = {
            ["elona.element_poison"] = 500,
         },
         image = 396,
         color = "Red",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.cobra",
      },
      {
         id = "spider",
         legacy_id = 127,
         item_type = 2,
         tags = { "wild" },
         level = 3,
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 1,
         ai_act_sub_freq = 20,
         creaturepack = 6,
         normal_actions = { -1 },
         special_actions = { 436 },
         can_talk = 1,
         original_relationship = "Aggressive",
         race = "elona.spider",
         resistances = {
            ["elona.element_poison"] = 500,
         },
         cspecialeq = 0,
         fltselect = 0,
         category = 6,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "black_widow",
         legacy_id = 128,
         item_type = 2,
         tags = { "wild" },
         level = 11,
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 1,
         ai_act_sub_freq = 15,
         creaturepack = 6,
         normal_actions = { -1 },
         special_actions = { 436 },
         can_talk = 1,
         original_relationship = "Aggressive",
         race = "elona.spider",
         element_of_unarmed_attack = 5500150,
         resistances = {
            ["elona.element_poison"] = 500,
         },
         color = "LightGreen",
         cspecialeq = 0,
         fltselect = 0,
         category = 6,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.cobra",
      },
      {
         id = "paralyzer",
         legacy_id = 129,
         item_type = 2,
         tags = { "wild" },
         level = 21,
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 1,
         ai_act_sub_freq = 15,
         creaturepack = 6,
         normal_actions = { -1 },
         special_actions = { 436 },
         can_talk = 1,
         original_relationship = "Aggressive",
         race = "elona.spider",
         element_of_unarmed_attack = 5800150,
         resistances = {
            ["elona.element_poison"] = 500,
         },
         color = "LightBlue",
         cspecialeq = 0,
         fltselect = 0,
         category = 6,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.cobra",
      },
      {
         id = "tarantula",
         legacy_id = 130,
         item_type = 2,
         tags = { "wild" },
         level = 15,
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 1,
         ai_act_sub_freq = 15,
         creaturepack = 6,
         normal_actions = { -1 },
         special_actions = { 436 },
         can_talk = 1,
         original_relationship = "Aggressive",
         race = "elona.spider",
         element_of_unarmed_attack = 5500200,
         resistances = {
            ["elona.element_poison"] = 500,
         },
         color = "Orange",
         cspecialeq = 0,
         fltselect = 0,
         category = 6,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.cobra",
      },
      {
         id = "blood_spider",
         legacy_id = 131,
         item_type = 2,
         tags = { "undead" },
         level = 28,
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 1,
         ai_act_sub_freq = 15,
         creaturepack = 6,
         normal_actions = { -1 },
         special_actions = { 436 },
         can_talk = 1,
         original_relationship = "Aggressive",
         race = "elona.spider",
         element_of_unarmed_attack = 5600100,
         resistances = {
            ["elona.element_poison"] = 500,
         },
         color = "PaleRed",
         cspecialeq = 0,
         fltselect = 0,
         category = 6,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "wooden_golem",
         legacy_id = 132,
         item_type = 3,
         tags = {},
         level = 13,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         original_relationship = "Aggressive",
         race = "elona.golem",
         color = "Yellow",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 40000,
         coefficient = 400,
         flags = { "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.iron",
      },
      {
         id = "stone_golem",
         legacy_id = 133,
         item_type = 3,
         tags = { "fire" },
         level = 19,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         original_relationship = "Aggressive",
         race = "elona.golem",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 40000,
         coefficient = 400,
         flags = { "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.iron",
      },
      {
         id = "steel_golem",
         legacy_id = 134,
         item_type = 3,
         tags = { "fire" },
         level = 25,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         original_relationship = "Aggressive",
         race = "elona.golem",
         class_ = "elona.predator",
         color = "LightGrey",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 40000,
         coefficient = 400,
         flags = { "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.iron",
      },
      {
         id = "golden_golem",
         legacy_id = 135,
         item_type = 3,
         tags = {},
         level = 30,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         original_relationship = "Aggressive",
         race = "elona.golem",
         class_ = "elona.predator",
         color = "Orange",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 30000,
         coefficient = 400,
         flags = { "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.iron",
      },
      {
         id = "mithril_golem",
         legacy_id = 136,
         item_type = 3,
         tags = {},
         level = 35,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         original_relationship = "Aggressive",
         race = "elona.golem",
         class_ = "elona.predator",
         color = "LightBlue",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 20000,
         coefficient = 400,
         flags = { "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.iron",
      },
      {
         id = "sky_golem",
         legacy_id = 137,
         item_type = 3,
         tags = {},
         level = 40,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         original_relationship = "Aggressive",
         race = "elona.golem",
         class_ = "elona.predator",
         color = "Cyan",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 15000,
         coefficient = 400,
         flags = { "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.iron",
      },
      {
         id = "adamantium_golem",
         legacy_id = 138,
         item_type = 3,
         tags = {},
         level = 50,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         original_relationship = "Aggressive",
         race = "elona.golem",
         class_ = "elona.predator",
         color = "Green",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 15000,
         coefficient = 400,
         flags = { "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.iron",
      },
      {
         id = "fire_crab",
         legacy_id = 147,
         item_type = 2,
         tags = { "fire" },
         level = 16,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.crab",
         element_of_unarmed_attack = 5000150,
         resistances = {
            ["elona.element_fire"] = 500,
         },
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.fire_crab",
      },
      {
         id = "fire_centipede",
         legacy_id = 148,
         item_type = 2,
         tags = { "fire" },
         level = 18,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.centipede",
         element_of_unarmed_attack = 5000200,
         resistances = {
            ["elona.element_fire"] = 500,
         },
         image = 239,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.fire_centipede",
      },
      {
         id = "cultist_of_fire",
         legacy_id = 149,
         item_type = 3,
         tags = { "fire" },
         level = 20,
         ai_calm = 1,
         ai_move = 85,
         ai_dist = 1,
         ai_act_sub_freq = 30,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 642, 450 },
         ai_heal = 400,
         original_relationship = "Aggressive",
         race = "elona.goblin",
         class_ = "elona.warmage",
         resistances = {
            ["elona.element_fire"] = 500,
         },
         image = 240,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "skeleton_warrior",
         legacy_id = 150,
         item_type = 3,
         tags = { "undead" },
         level = 12,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 9,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.skeleton",
         class_ = "elona.warrior",
         cspecialeq = 0,
         fltselect = 0,
         category = 9,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsImmuneToFear" },
      },
      {
         id = "skeleton_berserker",
         legacy_id = 151,
         item_type = 3,
         tags = { "undead" },
         level = 20,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 9,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.skeleton",
         class_ = "elona.warrior",
         color = "Red",
         cspecialeq = 1,
         eqtwohand = 1,
         fltselect = 0,
         category = 9,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsImmuneToFear" },
      },
      {
         id = "missionary_of_darkness",
         legacy_id = 152,
         item_type = 3,
         tags = { "man" },
         level = 20,
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 9,
         normal_actions = { -1, 613 },
         can_talk = 2,
         original_relationship = "Aggressive",
         race = "elona.eulderna",
         element_of_unarmed_attack = 5400150,
         image = 243,
         cspecialeq = 0,
         fltselect = 0,
         category = 9,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsImmuneToFear" },
      },
      {
         id = "pawn",
         legacy_id = 153,
         item_type = 3,
         tags = { "pawn" },
         level = 12,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.piece",
         class_ = "elona.warrior",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.iron",
      },
      {
         id = "rook",
         legacy_id = 154,
         item_type = 3,
         tags = { "pawn" },
         level = 16,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.piece",
         class_ = "elona.predator",
         image = 245,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.iron",
      },
      {
         id = "bishop",
         legacy_id = 155,
         item_type = 3,
         tags = { "pawn" },
         level = 18,
         ai_calm = 1,
         ai_move = 40,
         ai_dist = 2,
         ai_act_sub_freq = 20,
         creaturepack = 0,
         normal_actions = { -1, 414 },
         special_actions = { 410, 447 },
         original_relationship = "Aggressive",
         race = "elona.piece",
         class_ = "elona.wizard",
         image = 246,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.iron",
      },
      {
         id = "knight",
         legacy_id = 156,
         item_type = 3,
         tags = { "pawn" },
         level = 18,
         ai_calm = 1,
         ai_move = 30,
         ai_dist = 2,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.piece",
         class_ = "elona.warrior",
         image = 247,
         cspecialeq = 1,
         eqtwohand = 1,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.iron",
      },
      {
         id = "queen",
         legacy_id = 157,
         item_type = 3,
         tags = {},
         level = 22,
         ai_calm = 1,
         ai_move = 60,
         ai_dist = 1,
         ai_act_sub_freq = 20,
         creaturepack = 0,
         normal_actions = { -1, 415, 414 },
         special_actions = { 410 },
         original_relationship = "Aggressive",
         race = "elona.piece",
         class_ = "elona.wizard",
         image = 248,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.iron",
      },
      {
         id = "king",
         legacy_id = 158,
         item_type = 3,
         tags = {},
         level = 22,
         ai_calm = 1,
         ai_move = 40,
         ai_dist = 3,
         ai_act_sub_freq = 20,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 641 },
         original_relationship = "Aggressive",
         race = "elona.piece",
         class_ = "elona.warrior",
         image = 249,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.iron",
      },
      {
         id = "mercenary_warrior",
         legacy_id = 159,
         item_type = 3,
         tags = { "man", "shopguard" },
         level = 20,
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 8,
         normal_actions = { -1 },
         can_talk = 2,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.norland",
         class_ = "elona.warrior",
         image = 159,
         cspecialeq = 0,
         fltselect = 0,
         category = 8,
         rarity = 25000,
         coefficient = 400,
      },
      {
         id = "mercenary_archer",
         legacy_id = 160,
         item_type = 3,
         tags = { "man", "shopguard" },
         level = 20,
         ai_calm = 1,
         ai_move = 50,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 8,
         normal_actions = { -1 },
         can_talk = 2,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.norland",
         class_ = "elona.archer",
         image = 57,
         cspecialeq = 0,
         fltselect = 0,
         category = 8,
         rarity = 25000,
         coefficient = 400,
      },
      {
         id = "mercenary_wizard",
         legacy_id = 161,
         item_type = 3,
         tags = { "man", "shopguard" },
         level = 20,
         ai_calm = 1,
         ai_move = 50,
         ai_dist = 2,
         ai_act_sub_freq = 0,
         creaturepack = 8,
         normal_actions = { -1, 414 },
         can_talk = 2,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.norland",
         class_ = "elona.wizard",
         image = 100,
         cspecialeq = 0,
         fltselect = 0,
         category = 8,
         rarity = 25000,
         coefficient = 400,
      },
      {
         id = "rogue_boss",
         legacy_id = 302,
         item_type = 3,
         tags = { "man", "rogue" },
         level = 12,
         portrait = {
            male = "elona.man47",
         },
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 1,
         ai_act_sub_freq = 4,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 647 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Aggressive",
         race = "elona.juere",
         class_ = "elona.warrior",
         image = 150,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 25000,
         coefficient = 400,
         drops = { "elona.rogue", "elona.rogue_boss" },
         dialog_id = "elona.rogue_boss",
      },
      {
         id = "rogue_warrior",
         legacy_id = 303,
         item_type = 3,
         tags = { "man", "rogue" },
         level = 10,
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Aggressive",
         race = "elona.juere",
         class_ = "elona.thief",
         image = 31,
         cspecialeq = 1,
         eqtwohand = 1,
         fltselect = 0,
         category = 0,
         rarity = 25000,
         coefficient = 400,
         drops = { "elona.rogue" },
      },
      {
         id = "rogue_archer",
         legacy_id = 304,
         item_type = 3,
         tags = { "man", "rogue" },
         level = 10,
         ai_calm = 1,
         ai_move = 50,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -2 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Aggressive",
         race = "elona.juere",
         class_ = "elona.gunner",
         image = 158,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 25000,
         coefficient = 400,
         drops = { "elona.rogue" },
      },
      {
         id = "rogue_wizard",
         legacy_id = 305,
         item_type = 3,
         tags = { "man", "rogue" },
         level = 10,
         ai_calm = 1,
         ai_move = 50,
         ai_dist = 2,
         ai_act_sub_freq = 30,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 420, 414, 447 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Aggressive",
         race = "elona.juere",
         class_ = "elona.priest",
         image = 157,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 25000,
         coefficient = 400,
         drops = { "elona.rogue" },
      },
      {
         id = "yerles_machine_infantry",
         legacy_id = 162,
         item_type = 3,
         tags = { "man" },
         level = 5,
         ai_calm = 1,
         ai_move = 40,
         ai_dist = 3,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         original_relationship = "Aggressive",
         race = "elona.yerles",
         class_ = "elona.gunner",
         image = 250,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "yerles_elite_machine_infantry",
         legacy_id = 234,
         item_type = 3,
         tags = { "man" },
         level = 22,
         ai_calm = 1,
         ai_move = 40,
         ai_dist = 3,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         original_relationship = "Aggressive",
         race = "elona.yerles",
         class_ = "elona.gunner",
         image = 250,
         color = "Red",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "gilbert",
         legacy_id = 231,
         item_type = 3,
         tags = { "man" },
         level = 45,
         portrait = "elona.gilbert",
         ai_calm = 2,
         ai_move = 40,
         ai_dist = 3,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 4,
         original_relationship = "Neutral",
         race = "elona.juere",
         class_ = "elona.gunner",
         sex = "Male",
         image = 370,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         dialog_id = "elona.gilbert",
      },
      {
         id = "yerles_self_propelled_gun",
         legacy_id = 232,
         item_type = 3,
         tags = {},
         level = 17,
         ai_calm = 3,
         ai_move = 0,
         ai_dist = 4,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -2 },
         original_relationship = "Aggressive",
         race = "elona.yerles",
         class_ = "elona.gunner",
         image = 373,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "juere_infantry",
         legacy_id = 233,
         item_type = 3,
         tags = { "man" },
         level = 7,
         ai_calm = 1,
         ai_move = 95,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         original_relationship = "Aggressive",
         race = "elona.juere",
         class_ = "elona.warrior",
         image = 377,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "juere_swordman",
         legacy_id = 235,
         item_type = 3,
         tags = { "man" },
         level = 15,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         original_relationship = "Aggressive",
         race = "elona.juere",
         class_ = "elona.thief",
         image = 377,
         color = "Blue",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "rock_thrower",
         legacy_id = 163,
         item_type = 3,
         tags = { "man" },
         level = 9,
         ai_calm = 1,
         ai_move = 25,
         ai_dist = 3,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.yerles",
         class_ = "elona.thief",
         image = 251,
         cspecialeq = 1,
         eqrange_0 = 210,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "cat",
         legacy_id = 164,
         item_type = 1,
         tags = { "wild", "cat" },
         level = 4,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 4,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.cat",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsSuitableForMount" },
         corpse_eating_callback = "elona.eating_effect.cat",
      },
      {
         id = "silver_cat",
         legacy_id = 246,
         item_type = 1,
         tags = { "wild", "cat" },
         level = 3,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 4,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.cat",
         image = 387,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 1000,
         coefficient = 0,
         corpse_eating_callback = "elona.eating_effect.cat",
      },
      {
         id = "stray_cat",
         legacy_id = 332,
         item_type = 1,
         tags = { "wild", "cat" },
         level = 9,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 4,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.cat",
         image = 387,
         color = "Yellow",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 10000,
         coefficient = 0,
         corpse_eating_callback = "elona.eating_effect.cat",
      },
      {
         id = "lion",
         legacy_id = 229,
         item_type = 1,
         tags = { "wild", "cat" },
         level = 18,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 10,
         normal_actions = { -1 },
         can_talk = 4,
         original_relationship = "Aggressive",
         race = "elona.cat",
         class_ = "elona.predator",
         image = 368,
         cspecialeq = 0,
         fltselect = 0,
         category = 10,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsSuitableForMount" },
      },
      {
         id = "cacy",
         legacy_id = 230,
         item_type = 1,
         tags = { "wild", "cat" },
         level = 25,
         ai_calm = 1,
         ai_move = 50,
         ai_dist = 2,
         ai_act_sub_freq = 15,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 639 },
         can_talk = 4,
         original_relationship = "Aggressive",
         race = "elona.catgod",
         class_ = "elona.wizard",
         image = 369,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "carbuncle",
         legacy_id = 228,
         item_type = 1,
         tags = { "wild", "cat" },
         level = 20,
         ai_calm = 1,
         ai_move = 70,
         ai_dist = 1,
         ai_act_sub_freq = 20,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 436, 638 },
         can_talk = 4,
         original_relationship = "Aggressive",
         race = "elona.cat",
         class_ = "elona.wizard",
         image = 367,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "dog",
         legacy_id = 165,
         item_type = 1,
         tags = { "wild" },
         level = 4,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 10,
         normal_actions = { -1 },
         can_talk = 2,
         has_random_name = true,
         original_relationship = "Aggressive",
         race = "elona.dog",
         cspecialeq = 0,
         fltselect = 0,
         category = 10,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsSuitableForMount" },
      },
      {
         id = "poppy",
         legacy_id = 225,
         item_type = 1,
         tags = { "wild" },
         level = 1,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         original_relationship = "Neutral",
         race = "elona.dog",
         image = 364,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsSuitableForMount" },
         dialog_id = "elona.poppy",
      },
      {
         id = "rilian",
         legacy_id = 226,
         item_type = 3,
         tags = { "man" },
         level = 4,
         portrait = "elona.woman17",
         ai_calm = 2,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 4,
         original_relationship = "Neutral",
         race = "elona.roran",
         image = 365,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         dialog_id = "elona.rilian",
      },
      {
         id = "tam",
         legacy_id = 227,
         item_type = 3,
         tags = { "man" },
         level = 5,
         portrait = "elona.man17",
         ai_calm = 2,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         original_relationship = "Neutral",
         race = "elona.norland",
         sex = "Male",
         image = 366,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         dialog_id = "elona.tam",
      },
      {
         id = "little_girl",
         legacy_id = 166,
         item_type = 3,
         tags = { "man" },
         level = 4,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 2,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.roran",
         class_ = "elona.warrior",
         image = 4,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsSuitableForMount" },
      },
      {
         id = "rat",
         legacy_id = 167,
         item_type = 1,
         tags = { "wild" },
         level = 1,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         original_relationship = "Aggressive",
         race = "elona.rat",
         color = "Yellow",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "hermit_crab",
         legacy_id = 168,
         item_type = 1,
         tags = { "wild" },
         level = 1,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.shell",
         class_ = "elona.predator",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.calm",
         drops = { "elona.hermit_crab" },
      },
      {
         id = "public_performer",
         legacy_id = 170,
         item_type = 3,
         tags = { "man" },
         level = 1,
         ai_calm = 1,
         ai_move = 70,
         ai_dist = 2,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 2,
         original_relationship = "Aggressive",
         race = "elona.yerles",
         class_ = "elona.thief",
         image = 258,
         cspecialeq = 1,
         eqrange_0 = 210,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "frisia",
         legacy_id = 175,
         item_type = 3,
         tags = { "god" },
         level = 80,
         ai_calm = 1,
         ai_move = 50,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 2,
         original_relationship = "Aggressive",
         race = "elona.catgod",
         image = 348,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         drops = { "elona.god_boss" },
      },
      {
         id = "younger_sister",
         legacy_id = 176,
         item_type = 3,
         tags = { "man" },
         level = 1,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.roran",
         class_ = "elona.thief",
         image = 105,
         cspecialeq = 0,
         fltselect = 1,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsSuitableForMount" },
      },
      {
         id = "younger_sister_of_mansion",
         legacy_id = 249,
         item_type = 3,
         tags = { "man" },
         level = 50,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 100,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 643 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Neutral",
         race = "elona.roran",
         class_ = "elona.thief",
         image = 105,
         cspecialeq = 0,
         fltselect = 1,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsSuitableForMount" },
      },
      {
         id = "younger_cat_sister",
         legacy_id = 210,
         item_type = 3,
         tags = { "man" },
         level = 1,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.catsister",
         class_ = "elona.thief",
         cspecialeq = 0,
         fltselect = 1,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "young_lady",
         legacy_id = 211,
         item_type = 3,
         tags = { "man" },
         level = 1,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 10,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { -9998, 449, 447 },
         can_talk = 2,
         ai_heal = 404,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.roran",
         class_ = "elona.warmage",
         image = 355,
         cspecialeq = 0,
         fltselect = 1,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsSuitableForMount" },
      },
      {
         id = "utima",
         legacy_id = 177,
         item_type = 3,
         tags = { "god" },
         level = 80,
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 1,
         ai_act_sub_freq = 4,
         creaturepack = 0,
         normal_actions = { -2, -1, -1 },
         special_actions = { 647 },
         can_talk = 2,
         original_relationship = "Aggressive",
         race = "elona.machinegod",
         class_ = "elona.gunner",
         image = 349,
         fixlv = 6,
         cspecialeq = 1,
         eqweapon1 = 1,
         eqrange_0 = 514,
         eqammo_0 = 25030,
         eqammo_1 = 3,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         drops = { "elona.god_boss" },
      },
      {
         id = "azzrssil",
         legacy_id = 178,
         item_type = 6,
         tags = { "undead", "god" },
         level = 80,
         ai_calm = 1,
         ai_move = 60,
         ai_dist = 1,
         ai_act_sub_freq = 30,
         creaturepack = 0,
         normal_actions = { -1, 414, 419, 422 },
         special_actions = { 410, 443 },
         can_talk = 2,
         original_relationship = "Aggressive",
         race = "elona.undeadgod",
         class_ = "elona.wizard",
         sex = "Male",
         image = 339,
         color = "Red",
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.insanity",
         drops = { "elona.god_boss" },
      },
      {
         id = "master_of_pet_arena",
         legacy_id = 179,
         item_type = 3,
         tags = { "man" },
         level = 35,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.warrior",
         image = 350,
         cspecialeq = 0,
         fltselect = 1,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "garok",
         legacy_id = 208,
         item_type = 3,
         tags = { "man" },
         level = 45,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         original_relationship = "Neutral",
         race = "elona.dwarf",
         class_ = "elona.warrior",
         sex = "Male",
         image = 352,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         dialog_id = "elona.garok",
      },
      {
         id = "miral",
         legacy_id = 209,
         item_type = 3,
         tags = { "man" },
         level = 45,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         original_relationship = "Neutral",
         race = "elona.dwarf",
         class_ = "elona.warrior",
         sex = "Male",
         image = 353,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         dialog_id = "elona.miral",
      },
      {
         id = "twintail",
         legacy_id = 180,
         item_type = 1,
         tags = {},
         level = 1,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         original_relationship = "Neutral",
         race = "elona.cat",
         image = 259,
         cspecialeq = 0,
         fltselect = 4,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsSuitableForMount" },
      },
      {
         id = "silver_wolf",
         legacy_id = 181,
         item_type = 1,
         tags = { "god" },
         level = 10,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         original_relationship = "Neutral",
         race = "elona.hound",
         image = 260,
         cspecialeq = 0,
         fltselect = 4,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "nurse",
         legacy_id = 182,
         item_type = 3,
         tags = { "man" },
         level = 8,
         ai_calm = 1,
         ai_move = 50,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { 405 },
         can_talk = 2,
         has_random_name = true,
         original_relationship = "Aggressive",
         race = "elona.norland",
         class_ = "elona.wizard",
         image = 208,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsSuitableForMount" },
      },
      {
         id = "rich_person",
         legacy_id = 183,
         item_type = 3,
         tags = { "man" },
         level = 15,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Neutral",
         race = "elona.norland",
         male_image = 71,
         female_image = 70,
         cspecialeq = 0,
         fltselect = 7,
         category = 0,
         rarity = 20000,
         coefficient = 400,
      },
      {
         id = "noble_child",
         legacy_id = 184,
         item_type = 3,
         tags = { "man" },
         level = 9,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Neutral",
         race = "elona.norland",
         male_image = 73,
         female_image = 72,
         cspecialeq = 0,
         fltselect = 7,
         category = 0,
         rarity = 40000,
         coefficient = 400,
      },
      {
         id = "tourist",
         legacy_id = 185,
         item_type = 3,
         tags = { "man" },
         level = 20,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Neutral",
         race = "elona.yerles",
         class_ = "elona.gunner",
         male_image = 122,
         female_image = 129,
         cspecialeq = 0,
         fltselect = 7,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "festival_tourist",
         legacy_id = 350,
         item_type = 3,
         tags = { "man" },
         level = 10,
         portrait = "__random__",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 4,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.yerles",
         class_ = "elona.gunner",
         male_image = 477,
         female_image = 476,
         color = "White",
         cspecialeq = 0,
         fltselect = 1,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "blade",
         legacy_id = 186,
         item_type = 3,
         tags = {},
         level = 5,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.machine",
         element_of_unarmed_attack = 6100100,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.iron",
      },
      {
         id = "blade_alpha",
         legacy_id = 187,
         item_type = 3,
         tags = {},
         level = 13,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.machine",
         class_ = "elona.predator",
         element_of_unarmed_attack = 6100120,
         color = "LightBlue",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.iron",
      },
      {
         id = "blade_omega",
         legacy_id = 188,
         item_type = 3,
         tags = {},
         level = 30,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.machine",
         class_ = "elona.predator",
         element_of_unarmed_attack = 6100150,
         color = "Red",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.iron",
      },
      {
         id = "kaneda_bike",
         legacy_id = 345,
         item_type = 3,
         tags = {},
         level = 22,
         ai_calm = 3,
         ai_move = 0,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 4,
         original_relationship = "Neutral",
         race = "elona.bike",
         class_ = "elona.predator",
         image = 470,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsImmuneToFear", "IsSuitableForMount" },
         corpse_eating_callback = "elona.eating_effect.iron",
         dialog_id = "elona.kaneda_bike",
      },
      {
         id = "cub",
         legacy_id = 346,
         item_type = 3,
         tags = {},
         level = 8,
         ai_calm = 3,
         ai_move = 0,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 4,
         original_relationship = "Unconcerned",
         race = "elona.bike",
         class_ = "elona.predator",
         image = 471,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 5000,
         coefficient = 400,
         flags = { "IsImmuneToFear", "IsSuitableForMount" },
         corpse_eating_callback = "elona.eating_effect.iron",
      },
      {
         id = "mine_dog",
         legacy_id = 341,
         item_type = 3,
         tags = {},
         level = 15,
         ai_calm = 1,
         ai_move = 40,
         ai_dist = 3,
         ai_act_sub_freq = 15,
         creaturepack = 0,
         normal_actions = { 659, -4 },
         special_actions = { 466 },
         original_relationship = "Aggressive",
         race = "elona.machine",
         image = 434,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 25000,
         coefficient = 400,
         flags = { "IsImmuneToFear", "IsImmuneToMine", "CuresMpFrequently" },
         corpse_eating_callback = "elona.eating_effect.iron",
         drops = { "elona.mine_dog" },
      },
      {
         id = "iron_maiden",
         legacy_id = 258,
         item_type = 3,
         tags = {},
         level = 25,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.machine",
         element_of_unarmed_attack = 6100150,
         image = 198,
         cspecialeq = 0,
         damage_reaction_info = 250061,
         fltselect = 0,
         category = 0,
         rarity = 50000,
         coefficient = 400,
         flags = { "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.iron",
      },
      {
         id = "deformed_eye",
         legacy_id = 189,
         item_type = 3,
         tags = { "undead" },
         level = 8,
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 2,
         ai_act_sub_freq = 10,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 632 },
         original_relationship = "Aggressive",
         race = "elona.eye",
         image = 271,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 60000,
         coefficient = 400,
         flags = { "IsFloating", "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.deformed_eye",
         drops = { "elona.deformed_eye" },
      },
      {
         id = "impure_eye",
         legacy_id = 190,
         item_type = 3,
         tags = { "undead" },
         level = 19,
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 2,
         ai_act_sub_freq = 20,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 632 },
         original_relationship = "Aggressive",
         race = "elona.eye",
         image = 271,
         color = "Red",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 60000,
         coefficient = 400,
         flags = { "IsFloating", "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.deformed_eye",
         drops = { "elona.deformed_eye" },
      },
      {
         id = "wisp",
         legacy_id = 191,
         item_type = 3,
         tags = { "undead", "ether" },
         level = 14,
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 2,
         ai_act_sub_freq = 20,
         creaturepack = 0,
         normal_actions = { -1, 421, 604 },
         special_actions = { 633 },
         original_relationship = "Aggressive",
         race = "elona.wisp",
         resistances = {
            ["elona.element_lightning"] = 500,
         },
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 50000,
         coefficient = 400,
         flags = { "IsFloating", "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.ether",
      },
      {
         id = "hedgehog",
         legacy_id = 192,
         item_type = 1,
         tags = { "wild" },
         level = 5,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.crab",
         image = 273,
         cspecialeq = 0,
         damage_reaction_info = 200061,
         fltselect = 0,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "shining_hedgehog",
         legacy_id = 193,
         item_type = 1,
         tags = { "wild", "ether" },
         level = 15,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.crab",
         image = 273,
         color = "LightBlue",
         cspecialeq = 0,
         damage_reaction_info = 500062,
         fltselect = 0,
         category = 0,
         rarity = 70000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.ether",
      },
      {
         id = "chicken",
         legacy_id = 195,
         item_type = 1,
         tags = { "wild" },
         level = 1,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 2,
         original_relationship = "Aggressive",
         race = "elona.chicken",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 30000,
         coefficient = 400,
      },
      {
         id = "pumpkin",
         legacy_id = 196,
         item_type = 2,
         tags = { "undead" },
         level = 7,
         ai_calm = 3,
         ai_move = 50,
         ai_dist = 2,
         ai_act_sub_freq = 30,
         creaturepack = 0,
         normal_actions = { -3 },
         special_actions = { -9999 },
         original_relationship = "Aggressive",
         race = "elona.mandrake",
         element_of_unarmed_attack = 5400100,
         image = 275,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 60000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.pumpkin",
      },
      {
         id = "puppy",
         legacy_id = 201,
         item_type = 2,
         tags = { "undead" },
         level = 5,
         ai_calm = 3,
         ai_move = 80,
         ai_dist = 3,
         ai_act_sub_freq = 10,
         creaturepack = 0,
         normal_actions = { -3 },
         special_actions = { -9999 },
         original_relationship = "Aggressive",
         race = "elona.mandrake",
         element_of_unarmed_attack = 5400150,
         image = 275,
         color = "Orange",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 20000,
         coefficient = 400,
         flags = { "IsInvisible" },
         corpse_eating_callback = "elona.eating_effect.pumpkin",
      },
      {
         id = "greater_pumpkin",
         legacy_id = 197,
         item_type = 2,
         tags = { "undead" },
         level = 18,
         ai_calm = 3,
         ai_move = 50,
         ai_dist = 2,
         ai_act_sub_freq = 30,
         creaturepack = 0,
         normal_actions = { -3 },
         special_actions = { -9998 },
         original_relationship = "Aggressive",
         race = "elona.mandrake",
         element_of_unarmed_attack = 5400200,
         image = 275,
         color = "Blue",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 60000,
         coefficient = 400,
         flags = { "IsInvisible" },
         corpse_eating_callback = "elona.eating_effect.greater_pumpkin",
      },
      {
         id = "halloween_nightmare",
         legacy_id = 198,
         item_type = 2,
         tags = { "undead" },
         level = 30,
         ai_calm = 3,
         ai_move = 50,
         ai_dist = 2,
         ai_act_sub_freq = 30,
         creaturepack = 0,
         normal_actions = { -3 },
         special_actions = { -9997 },
         original_relationship = "Aggressive",
         race = "elona.mandrake",
         element_of_unarmed_attack = 5400250,
         image = 275,
         color = "Red",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 60000,
         coefficient = 400,
         flags = { "IsInvisible" },
         corpse_eating_callback = "elona.eating_effect.halloween_nightmare",
      },
      {
         id = "stalker",
         legacy_id = 199,
         item_type = 3,
         tags = { "undead" },
         level = 12,
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 2,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.stalker",
         class_ = "elona.predator",
         image = 276,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 30000,
         coefficient = 400,
         flags = { "IsInvisible", "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.stalker",
      },
      {
         id = "shadow_stalker",
         legacy_id = 200,
         item_type = 3,
         tags = { "undead" },
         level = 26,
         ai_calm = 1,
         ai_move = 70,
         ai_dist = 2,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.stalker",
         class_ = "elona.predator",
         image = 276,
         color = "Red",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 30000,
         coefficient = 400,
         flags = { "IsInvisible", "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.stalker",
      },
      {
         id = "ebon",
         legacy_id = 202,
         item_type = 3,
         tags = {},
         level = 80,
         ai_calm = 3,
         ai_move = 80,
         ai_dist = 1,
         ai_act_sub_freq = 65,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 602 },
         can_talk = 2,
         original_relationship = "Neutral",
         race = "elona.giant",
         resistances = {
            ["elona.element_fire"] = 500,
         },
         sex = "Male",
         image = 351,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "moyer",
         legacy_id = 203,
         item_type = 3,
         tags = { "man" },
         level = 10,
         portrait = "__random__",
         ai_calm = 3,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 2,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.gunner",
         sex = "Male",
         image = 23,
         cspecialeq = 0,
         fltselect = 1,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "maid",
         legacy_id = 205,
         item_type = 3,
         tags = { "man" },
         level = 1,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Neutral",
         race = "elona.yerles",
         class_ = "elona.tourist",
         male_image = 104,
         female_image = 105,
         cspecialeq = 0,
         fltselect = 1,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "ebon2",
         legacy_id = 207,
         item_type = 3,
         tags = { "god" },
         level = 30,
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 1,
         ai_act_sub_freq = 65,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 602 },
         can_talk = 2,
         original_relationship = "Aggressive",
         race = "elona.giant",
         resistances = {
            ["elona.element_fire"] = 500,
         },
         sex = "Male",
         image = 351,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      -- "Has" the same name as "Stersha" in "EN", but is named "実験台" in "JP".
      {
         id = "test_subject",
         legacy_id = 212,
         item_type = 3,
         tags = {},
         level = 1,
         portrait = "elona.stersha",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.juere",
         class_ = "elona.warrior",
         sex = "Female",
         image = 117,
         fixlv = 6,
         cspecialeq = 1,
         eqweapon1 = 56,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "gwen",
         legacy_id = 213,
         item_type = 3,
         tags = { "man" },
         level = 1,
         ai_calm = 4,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         original_relationship = "Unconcerned",
         race = "elona.roran",
         class_ = "elona.warmage",
         sex = "Female",
         image = 356,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         drops = { "elona.gwen" },
      },
      {
         id = "pael",
         legacy_id = 221,
         item_type = 3,
         tags = { "man" },
         level = 10,
         portrait = "elona.woman16",
         ai_calm = 3,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 2,
         original_relationship = "Neutral",
         race = "elona.roran",
         class_ = "elona.thief",
         sex = "Female",
         image = 358,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         dialog_id = "elona.pael",
      },
      {
         id = "lily",
         legacy_id = 222,
         item_type = 3,
         tags = { "man" },
         level = 15,
         portrait = "elona.woman14",
         ai_calm = 3,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 2,
         original_relationship = "Neutral",
         race = "elona.roran",
         class_ = "elona.warrior",
         sex = "Female",
         image = 359,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         dialog_id = "elona.lily",
      },
      {
         id = "raphael",
         legacy_id = 223,
         item_type = 3,
         tags = { "man" },
         level = 10,
         portrait = "elona.man15",
         ai_calm = 2,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 4,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.warrior",
         sex = "Male",
         image = 361,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         dialog_id = "elona.raphael",
      },
      {
         id = "ainc",
         legacy_id = 224,
         item_type = 3,
         tags = { "man" },
         level = 7,
         portrait = "elona.man19",
         ai_calm = 3,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 4,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.warrior",
         sex = "Male",
         image = 362,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         dialog_id = "elona.ainc",
      },
      {
         id = "arnord",
         legacy_id = 243,
         item_type = 3,
         tags = { "man" },
         level = 15,
         portrait = "elona.arnord",
         ai_calm = 3,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 4,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.gunner",
         sex = "Male",
         image = 383,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         dialog_id = "elona.arnord",
      },
      {
         id = "mia",
         legacy_id = 247,
         item_type = 3,
         tags = { "man" },
         level = 4,
         portrait = "elona.mia",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         original_relationship = "Neutral",
         race = "elona.norland",
         sex = "Female",
         image = 388,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         dialog_id = "elona.mia",
      },
      {
         id = "renton",
         legacy_id = 252,
         item_type = 3,
         tags = { "man" },
         level = 45,
         portrait = "elona.man6",
         ai_calm = 1,
         ai_move = 50,
         ai_dist = 2,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { 417, 434, 415, 454 },
         can_talk = 4,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.wizard",
         sex = "Male",
         image = 392,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         dialog_id = "elona.renton",
      },
      {
         id = "marks",
         legacy_id = 253,
         item_type = 3,
         tags = { "man" },
         level = 25,
         portrait = "elona.man2",
         ai_calm = 1,
         ai_move = 70,
         ai_dist = 1,
         ai_act_sub_freq = 25,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 635 },
         can_talk = 4,
         original_relationship = "Neutral",
         race = "elona.juere",
         class_ = "elona.thief",
         sex = "Male",
         image = 394,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         dialog_id = "elona.marks",
      },
      {
         id = "noel",
         legacy_id = 259,
         item_type = 3,
         tags = { "man" },
         level = 20,
         portrait = "elona.woman16",
         ai_calm = 1,
         ai_move = 70,
         ai_dist = 1,
         ai_act_sub_freq = 25,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 635 },
         can_talk = 4,
         original_relationship = "Neutral",
         race = "elona.juere",
         class_ = "elona.thief",
         sex = "Female",
         image = 199,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         dialog_id = "elona.noel",
      },
      {
         id = "conery",
         legacy_id = 301,
         item_type = 3,
         tags = { "man" },
         level = 38,
         portrait = "elona.conery",
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 4,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.warrior",
         sex = "Male",
         image = 403,
         fixlv = 6,
         cspecialeq = 1,
         eqtwohand = 1,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsQuickTempered" },
         dialog_id = "elona.conery",
      },
      {
         id = "thief",
         legacy_id = 214,
         item_type = 3,
         tags = { "man" },
         level = 2,
         ai_calm = 1,
         ai_move = 70,
         ai_dist = 1,
         ai_act_sub_freq = 25,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 635 },
         can_talk = 3,
         original_relationship = "Aggressive",
         race = "elona.juere",
         image = 166,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 30000,
         coefficient = 400,
         flags = { "DropsGold" },
      },
      {
         id = "robber",
         legacy_id = 215,
         item_type = 3,
         tags = { "man" },
         level = 5,
         ai_calm = 1,
         ai_move = 70,
         ai_dist = 1,
         ai_act_sub_freq = 25,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 635 },
         can_talk = 3,
         original_relationship = "Aggressive",
         race = "elona.juere",
         class_ = "elona.thief",
         image = 166,
         color = "Red",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 30000,
         coefficient = 400,
         flags = { "DropsGold" },
      },
      {
         id = "master_thief",
         legacy_id = 217,
         item_type = 3,
         tags = { "man" },
         level = 35,
         ai_calm = 1,
         ai_move = 70,
         ai_dist = 1,
         ai_act_sub_freq = 25,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 635 },
         can_talk = 3,
         original_relationship = "Aggressive",
         race = "elona.juere",
         class_ = "elona.thief",
         image = 166,
         color = "Blue",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 30000,
         coefficient = 400,
         flags = { "DropsGold" },
      },
      {
         id = "great_race_of_yith",
         legacy_id = 216,
         item_type = 1,
         tags = {},
         level = 50,
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1, 601, 636 },
         original_relationship = "Aggressive",
         race = "elona.yith",
         class_ = "elona.predator",
         resistances = {
            ["elona.element_mind"] = 500,
         },
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 20000,
         coefficient = 400,
         flags = { "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.yith",
      },
      {
         id = "shub_niggurath",
         legacy_id = 218,
         item_type = 1,
         tags = {},
         level = 45,
         ai_calm = 3,
         ai_move = 0,
         ai_dist = 3,
         ai_act_sub_freq = 10,
         creaturepack = 0,
         normal_actions = { -3, -3, 410, 636 },
         special_actions = { 424 },
         original_relationship = "Aggressive",
         race = "elona.yith",
         class_ = "elona.predator",
         resistances = {
            ["elona.element_mind"] = 500,
         },
         image = 277,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 40000,
         coefficient = 400,
         flags = { "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.yith",
      },
      {
         id = "gagu",
         legacy_id = 219,
         item_type = 3,
         tags = {},
         level = 38,
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 1,
         ai_act_sub_freq = 20,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 613 },
         original_relationship = "Aggressive",
         race = "elona.orc",
         class_ = "elona.warrior",
         image = 278,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 80000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.yith",
      },
      {
         id = "spiral_king",
         legacy_id = 220,
         item_type = 3,
         tags = { "god" },
         level = 65,
         ai_calm = 1,
         ai_move = 50,
         ai_dist = 2,
         ai_act_sub_freq = 10,
         creaturepack = 0,
         normal_actions = { 417, 434, 415, 454 },
         special_actions = { 636 },
         original_relationship = "Aggressive",
         race = "elona.yith",
         class_ = "elona.wizard",
         resistances = {
            ["elona.element_mind"] = 500,
         },
         image = 430,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 30000,
         coefficient = 400,
         flags = { "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.yith",
      },
      {
         id = "fairy",
         legacy_id = 250,
         item_type = 3,
         tags = {},
         level = 13,
         ai_calm = 1,
         ai_move = 60,
         ai_dist = 1,
         ai_act_sub_freq = 40,
         creaturepack = 0,
         normal_actions = { -1, 423 },
         special_actions = { 410, 635 },
         original_relationship = "Aggressive",
         race = "elona.fairy",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 50000,
         coefficient = 400,
         flags = { "IsFloating", "DropsGold" },
         drops = { "elona.fairy" },
      },
      {
         id = "black_cat",
         legacy_id = 260,
         item_type = 3,
         tags = { "god" },
         level = 8,
         ai_calm = 1,
         ai_move = 75,
         ai_dist = 1,
         ai_act_sub_freq = 30,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 414, 601, 636 },
         can_talk = 4,
         has_random_name = true,
         original_relationship = "Aggressive",
         race = "elona.servant",
         class_ = "elona.thief",
         image = 211,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 50000,
         coefficient = 400,
         flags = { "IsFloating" },
      },
      {
         id = "cute_fairy",
         legacy_id = 261,
         item_type = 3,
         tags = { "god" },
         level = 8,
         ai_calm = 1,
         ai_move = 40,
         ai_dist = 2,
         ai_act_sub_freq = 20,
         creaturepack = 0,
         normal_actions = { -2 },
         special_actions = { 443, 447, 415 },
         can_talk = 1,
         has_random_name = true,
         original_relationship = "Aggressive",
         race = "elona.servant",
         class_ = "elona.archer",
         image = 217,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 50000,
         coefficient = 400,
         flags = { "IsFloating" },
      },
      {
         id = "android",
         legacy_id = 262,
         item_type = 3,
         tags = { "god" },
         level = 8,
         ai_calm = 1,
         ai_move = 40,
         ai_dist = 2,
         ai_act_sub_freq = 3,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 647 },
         has_random_name = true,
         original_relationship = "Aggressive",
         race = "elona.servant",
         class_ = "elona.gunner",
         image = 218,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 50000,
         coefficient = 400,
         flags = { "IsFloating" },
      },
      {
         id = "black_angel",
         legacy_id = 263,
         item_type = 3,
         tags = { "god" },
         level = 8,
         ai_calm = 1,
         ai_move = 70,
         ai_dist = 1,
         ai_act_sub_freq = 8,
         creaturepack = 0,
         normal_actions = { -1, -1, 648, -2 },
         special_actions = { 446, 647, 447 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Aggressive",
         race = "elona.servant",
         class_ = "elona.archer",
         sex = "Female",
         image = 236,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 50000,
         coefficient = 400,
         flags = { "IsFloating", "IsSuitableForMount" },
      },
      {
         id = "exile",
         legacy_id = 264,
         item_type = 3,
         tags = { "god" },
         level = 8,
         ai_calm = 1,
         ai_move = 65,
         ai_dist = 2,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { 414, 415, 418 },
         has_random_name = true,
         original_relationship = "Aggressive",
         race = "elona.servant",
         class_ = "elona.wizard",
         image = 269,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 50000,
         coefficient = 400,
         flags = { "IsFloating", "CanCastRapidMagic" },
      },
      {
         id = "golden_knight",
         legacy_id = 265,
         item_type = 3,
         tags = { "god" },
         level = 8,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 8,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 648, 444 },
         ai_heal = 401,
         has_random_name = true,
         original_relationship = "Aggressive",
         race = "elona.servant",
         class_ = "elona.warrior",
         sex = "Female",
         image = 267,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 50000,
         coefficient = 400,
         flags = { "IsFloating" },
      },
      {
         id = "defender",
         legacy_id = 266,
         item_type = 3,
         tags = { "god" },
         level = 8,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 8,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 442, 444 },
         ai_heal = 404,
         has_random_name = true,
         original_relationship = "Aggressive",
         race = "elona.servant",
         class_ = "elona.warmage",
         sex = "Male",
         image = 242,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 50000,
         coefficient = 400,
         flags = { "IsFloating", "HasLayHand" },
      },
      {
         id = "lame_horse",
         legacy_id = 267,
         item_type = 1,
         tags = { "wild", "horse" },
         level = 1,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.horse",
         color = "LightBlue",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 10000,
         coefficient = 400,
         flags = { "IsSuitableForMount" },
         corpse_eating_callback = "elona.eating_effect.horse",
      },
      {
         id = "wild_horse",
         legacy_id = 276,
         item_type = 1,
         tags = { "wild", "horse" },
         level = 4,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.horse",
         class_ = "elona.predator",
         color = "Yellow",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 10000,
         coefficient = 400,
         flags = { "IsSuitableForMount" },
         corpse_eating_callback = "elona.eating_effect.horse",
      },
      {
         id = "noyel_horse",
         legacy_id = 275,
         item_type = 1,
         tags = { "wild", "horse" },
         level = 10,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.horse",
         class_ = "elona.predator",
         color = "LightGrey",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 10000,
         coefficient = 400,
         flags = { "IsSuitableForMount" },
         corpse_eating_callback = "elona.eating_effect.horse",
      },
      {
         id = "yowyn_horse",
         legacy_id = 268,
         item_type = 1,
         tags = { "wild", "horse" },
         level = 15,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.horse",
         class_ = "elona.predator",
         color = "PaleRed",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 10000,
         coefficient = 400,
         flags = { "IsSuitableForMount" },
         corpse_eating_callback = "elona.eating_effect.horse",
      },
      {
         id = "wild_horse2",
         legacy_id = 277,
         item_type = 1,
         tags = { "wild", "horse" },
         level = 20,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         has_random_name = true,
         original_relationship = "Unconcerned",
         race = "elona.horse",
         class_ = "elona.predator",
         color = "LightRed",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 10000,
         coefficient = 400,
         flags = { "IsSuitableForMount" },
         corpse_eating_callback = "elona.eating_effect.horse",
      },
      {
         id = "mutant",
         legacy_id = 278,
         item_type = 3,
         tags = {},
         level = 6,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         has_random_name = true,
         original_relationship = "Aggressive",
         race = "elona.mutant",
         class_ = "elona.warrior",
         image = 191,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 70000,
         coefficient = 400,
      },
      {
         id = "icolle",
         legacy_id = 279,
         item_type = 3,
         tags = { "man" },
         level = 15,
         portrait = "elona.man18",
         ai_calm = 1,
         ai_move = 70,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 2,
         original_relationship = "Neutral",
         race = "elona.yerles",
         class_ = "elona.gunner",
         sex = "Male",
         image = 263,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         dialog_id = "elona.icolle",
      },
      {
         id = "balzak",
         legacy_id = 280,
         item_type = 3,
         tags = { "man" },
         level = 10,
         portrait = "elona.balzak",
         ai_calm = 5,
         ai_move = 70,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 3,
         original_relationship = "Neutral",
         race = "elona.norland",
         sex = "Male",
         image = 192,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         dialog_id = "elona.balzak",
      },
      {
         id = "revlus",
         legacy_id = 288,
         item_type = 3,
         tags = { "man" },
         level = 55,
         portrait = "elona.man14",
         ai_calm = 1,
         ai_move = 65,
         ai_dist = 2,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { 414, 415, 418 },
         can_talk = 4,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.wizard",
         sex = "Male",
         image = 193,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsFloating", "CanCastRapidMagic" },
      },
      {
         id = "lexus",
         legacy_id = 290,
         item_type = 3,
         tags = { "man" },
         level = 38,
         portrait = "elona.man6",
         ai_calm = 3,
         ai_move = 65,
         ai_dist = 2,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { 414, 415, 418 },
         can_talk = 1,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.wizard",
         sex = "Male",
         image = 196,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         flags = { "IsFloating", "CanCastRapidMagic" },
         dialog_id = "elona.lexus",
      },
      {
         id = "sin",
         legacy_id = 292,
         item_type = 3,
         tags = { "man" },
         level = 55,
         portrait = "elona.man7",
         ai_calm = 1,
         ai_move = 65,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Neutral",
         race = "elona.juere",
         class_ = "elona.thief",
         sex = "Male",
         image = 197,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "abyss",
         legacy_id = 294,
         item_type = 3,
         tags = { "man" },
         level = 38,
         portrait = "elona.man6",
         ai_calm = 3,
         ai_move = 65,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         original_relationship = "Neutral",
         race = "elona.juere",
         class_ = "elona.thief",
         sex = "Male",
         image = 196,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         dialog_id = "elona.abyss",
      },
      {
         id = "fray",
         legacy_id = 291,
         item_type = 3,
         tags = { "man" },
         level = 55,
         portrait = "elona.woman18",
         ai_calm = 1,
         ai_move = 90,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.warrior",
         sex = "Female",
         image = 80,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
      },
      {
         id = "doria",
         legacy_id = 297,
         item_type = 3,
         tags = { "man" },
         level = 38,
         portrait = "elona.man6",
         ai_calm = 3,
         ai_move = 90,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 1,
         original_relationship = "Neutral",
         race = "elona.norland",
         class_ = "elona.warrior",
         sex = "Male",
         image = 196,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 100000,
         coefficient = 400,
         dialog_id = "elona.doria",
      },
      {
         id = "silver_eyed_witch",
         legacy_id = 317,
         item_type = 3,
         tags = {},
         level = 28,
         ai_calm = 1,
         ai_move = 80,
         ai_dist = 1,
         ai_act_sub_freq = 10,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 648 },
         can_talk = 4,
         has_random_name = true,
         original_relationship = "Aggressive",
         race = "elona.roran",
         class_ = "elona.claymore",
         sex = "Female",
         image = 279,
         cspecialeq = 1,
         eqweapon1 = 232,
         eqtwohand = 1,
         fltselect = 0,
         category = 0,
         rarity = 50000,
         coefficient = 400,
      },
      {
         id = "big_daddy",
         legacy_id = 318,
         item_type = 3,
         tags = {},
         level = 30,
         ai_calm = 1,
         ai_move = 25,
         ai_dist = 3,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 4,
         original_relationship = "Neutral",
         race = "elona.machinegod",
         class_ = "elona.gunner",
         sex = "Male",
         image = 408,
         fixlv = 6,
         cspecialeq = 1,
         eqrange_0 = 496,
         eqrange_1 = 4,
         eqammo_0 = 25020,
         eqammo_1 = 3,
         fltselect = 3,
         category = 0,
         rarity = 50000,
         coefficient = 400,
         flags = { "IsImmuneToFear" },
         corpse_eating_callback = "elona.eating_effect.iron",
      },
      {
         id = "little_sister",
         legacy_id = 319,
         item_type = 3,
         tags = {},
         level = 1,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 5,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 4,
         original_relationship = "Neutral",
         race = "elona.mutant",
         class_ = "elona.tourist",
         sex = "Female",
         image = 409,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 50000,
         coefficient = 400,
      },
      {
         id = "strange_scientist",
         legacy_id = 322,
         item_type = 3,
         tags = { "man" },
         level = 15,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 3,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 4,
         original_relationship = "Neutral",
         race = "elona.roran",
         class_ = "elona.gunner",
         sex = "Female",
         image = 412,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 50000,
         coefficient = 400,
         dialog_id = "elona.strange_scientist",
      },
      {
         id = "mysterious_producer",
         legacy_id = 334,
         item_type = 3,
         tags = { "man" },
         level = 7,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 3,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         can_talk = 4,
         original_relationship = "Neutral",
         race = "elona.juere",
         class_ = "elona.tourist",
         sex = "Male",
         image = 416,
         fixlv = 6,
         cspecialeq = 0,
         fltselect = 3,
         category = 0,
         rarity = 50000,
         coefficient = 400,
      },
      {
         id = "shade",
         legacy_id = 323,
         item_type = 3,
         tags = { "undead" },
         level = 12,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1, 617, 613 },
         original_relationship = "Aggressive",
         race = "elona.ghost",
         element_of_unarmed_attack = 5600400,
         image = 280,
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 10000,
         coefficient = 0,
         flags = { "IsFloating" },
         corpse_eating_callback = "elona.eating_effect.ghost",
      },
      {
         id = "quickling",
         legacy_id = 324,
         item_type = 3,
         tags = {},
         level = 10,
         ai_calm = 1,
         ai_move = 50,
         ai_dist = 2,
         ai_act_sub_freq = 0,
         creaturepack = 0,
         normal_actions = { -1 },
         original_relationship = "Aggressive",
         race = "elona.quickling",
         color = "LightGreen",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 15000,
         coefficient = 400,
         flags = { "IsUnsuitableForMount", "IsImmuneToElementalDamage" },
         corpse_eating_callback = "elona.eating_effect.quickling",
      },
      {
         id = "quickling_archer",
         legacy_id = 325,
         item_type = 3,
         tags = {},
         level = 17,
         ai_calm = 1,
         ai_move = 50,
         ai_dist = 3,
         ai_act_sub_freq = 2,
         creaturepack = 0,
         normal_actions = { -1, -3, -3 },
         special_actions = { 648 },
         original_relationship = "Aggressive",
         race = "elona.quickling",
         class_ = "elona.archer",
         color = "Orange",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 15000,
         coefficient = 400,
         flags = { "IsUnsuitableForMount", "IsImmuneToElementalDamage" },
         corpse_eating_callback = "elona.eating_effect.quickling",
      },
      {
         id = "silver_bell",
         legacy_id = 328,
         item_type = 1,
         tags = {},
         level = 3,
         ai_calm = 1,
         ai_move = 30,
         ai_dist = 4,
         ai_act_sub_freq = 1,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 653 },
         can_talk = 2,
         original_relationship = "Aggressive",
         race = "elona.metal",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 2000,
         coefficient = 0,
         flags = { "IsUnsuitableForMount", "IsMetal", "IsImmuneToElementalDamage", "IsFloating" },
         corpse_eating_callback = "elona.eating_effect.iron",
         drops = { "elona.silver_bell" },
      },
      {
         id = "gold_bell",
         legacy_id = 329,
         item_type = 1,
         tags = {},
         level = 1,
         ai_calm = 1,
         ai_move = 30,
         ai_dist = 3,
         ai_act_sub_freq = 1,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 653 },
         can_talk = 2,
         original_relationship = "Aggressive",
         race = "elona.metal",
         color = "Orange",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 5000,
         coefficient = 0,
         flags = { "IsUnsuitableForMount", "IsMetal", "IsImmuneToElementalDamage", "IsFloating" },
         corpse_eating_callback = "elona.eating_effect.iron",
         drops = { "elona.gold_bell" },
      },
      {
         id = "alien",
         legacy_id = 330,
         item_type = 1,
         tags = { "dragon" },
         level = 19,
         ai_calm = 1,
         ai_move = 100,
         ai_dist = 1,
         ai_act_sub_freq = 7,
         creaturepack = 0,
         normal_actions = { -1 },
         special_actions = { 654 },
         original_relationship = "Aggressive",
         race = "elona.dinosaur",
         class_ = "elona.predator",
         element_of_unarmed_attack = 6300250,
         image = 283,
         color = "LightBlue",
         cspecialeq = 0,
         fltselect = 0,
         category = 0,
         rarity = 40000,
         coefficient = 400,
         corpse_eating_callback = "elona.eating_effect.alien",
      },
      -- For debug
      -- chara spiral_putit {
      --     id = 500,
      --     item_type = 1,
      --     tags = { "slime" },
      --     level = 1,
      --     ai_calm = 1,
      --     ai_move = 50,
      --     ai_dist = 2,
      --     ai_act_sub_freq = 0,
      --     creaturepack = 3,
      --     normal_actions = { 417, 434, 415, 454 },
      --     special_actions = { 636 },
      --     original_relationship = "Aggressive",
      --     race = "elona.slime",
      --     class_ = "elona.wizard",
      --     resistances = {
      --        ["elona.element_mind"] = 500,
      --     },
      --     image = 430,
      --     cspecialeq = 0,
      --     fltselect = 0,
      --     category = 3,
      --     rarity = 0,
      --     coefficient = 0,
      -- },
      --
      -- For debug
      -- chara dragon_putit {
      --     id = 501,
      --     item_type = 5,
      --     tags = { "dragon" },
      --     level = 1,
      --     ai_calm = 1,
      --     ai_move = 100,
      --     ai_dist = 1,
      --     creaturepack = 0,
      --     normal_actions = { 602, 603, 604, 605, 606, 607, 608, 609, 610, 611, 612 },
      --     original_relationship = "Aggressive",
      --     race = "elona.dragon",
      --     class_ = "elona.predator",
      --     cspecialeq = 0,
      --     fltselect = 0,
      --     category = 0,
      --     rarity = 0,
      --     coefficient = 0,
      -- },
})
