...
i18n.add {
   ui = {
      ...
      assign_shortcut = "{{{$1}}}キーにショートカットを割り当てた。",
      ...
      chara_sheet = {
         ...
         you_can_spend_bonus = "残り {$1} のボーナスをスキルに分配できる",
      ...
      equip = {
         ...
         cannot_be_taken_off = "{itemname($1)}は外せない。",
         you_unequip = "{itemname($1)}を外した。",
      },
      ...
      journal = {
         ...
         income = {
            ...
            bills = {
               title = "◆ 請求書内訳(毎月1日に発行)",
               labor = "@RE　人件費  : 約 {$1} gold",
               maintenance = "@RE　運営費  : 約 {$1} gold",
               tax = "@RE　税金    : 約 {$1} gold",
               sum = "@RE　合計　  : 約 {$1} gold",
               unpaid = "現在未払いの請求書は{$1}枚",
            },
         },
      },
      ...
      politics = {
         ...
         name = "この国の首都は{$1}だ。",
         taxes = "この街の消費税は{$1}%だ。",
         well_pollution = "この街の井戸水の汚染は深刻だ(死者{$1}人）。",
      ...
      autopick = {
         do_you_really_pick_up = "{name($1)}を拾う？",
         do_you_really_destroy = "{name($1)}を破壊する？",
         destroyed = "{name($1)}を破壊した。",
         do_you_really_open = "{name($1)}を開ける？",
      },
