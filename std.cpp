...
std::string operator+(elona_vector1<std::string>& lhs, elona_vector1<std::string>& rhs)
{
    return lhs(0) + rhs(0);
}
...
{
    LOG("copy", from, to);
    fs::copy_file(to_unix_filename(from), to_unix_filename(to), fs::copy_options::overwrite_existing);
}
...
void bsave(const std::string& filename, const std::string& data)
{
    ...
    out.write(reinterpret_cast<const char*>(data.c_str()), sizeof(std::size(data)));
}
...
void buffer(int window_id, int width, int height)
{
    ...
    if (auto texture = detail::tex_buffers[window_id].texture)
    {
        int img_width;
        int img_height;
        snail::detail::enforce_sdl(::SDL_QueryTexture(texture, NULL, NULL, &img_width, &img_height));
    ...
    Application::instance().register_finalizer(
            [ptr = detail::tex_buffers[window_id].texture]() { ::SDL_DestroyTexture(ptr); });

    const auto save = Application::instance().renderer().get_render_target();
    Application::instance().renderer().set_render_target(detail::tex_buffers[window_id].texture);
    Application::instance().renderer().set_draw_color({0, 0, 0, 0});
    Application::instance().renderer().clear();
...
void color(int r, int g, int b)
{
    ...
    Application::instance().renderer().set_draw_color(detail::current_tex_buffer().color);
}
...
{
    for (auto pos = s.find(from); pos != std::string::npos; pos = s.find(from, pos + std::string::traits_type::length(to)))
    {
...
void dirlist(std::string& out, const std::string& glob, int attr)
{
    ...
    for (const auto& dir : fs::directory_iterator(fs::path{to_unix_filename(glob)}.parent_path()))
    {
        if (cond(dir.path()))
        {
            out += dir.path().string();
...
{
    if (fs::exists(to_unix_filename(filename)) && !fs::is_directory(to_unix_filename(filename)))
    {
...
{
    if (auto i = font_detail::font_cache.find(size); i != std::end(font_detail::font_cache))
    {
    ...
    else
    {
        const auto [i_, _] = font_detail::font_cache.emplace(
                ...
                std::forward_as_tuple("font/APJapanesefontT.ttf", size, Font::Style::regular));
...
void gcopy(int window_id, int src_x, int src_y, int src_width, int src_height)
{
    ...
    snail::detail::enforce_sdl(::SDL_SetTextureAlphaMod(detail::tex_buffers[window_id].texture, detail::current_tex_buffer().color.a));
    
    if (window_id == detail::current_buffer)
    {
        src_width = src_width == 0 ? detail::current_tex_buffer().width : src_width,
        src_height = src_height == 0 ? detail::current_tex_buffer().height : src_height,
        Application::instance().renderer().set_render_target(detail::tmp_buffer);
        if (window_id < 10)
        {
            Application::instance().renderer().set_blend_mode(BlendMode::none);
            Application::instance().renderer().set_draw_color({0, 0, 0, 0});
...
void getstr(std::string& out, const std::string& source, int offset, char delimiter, int limit)
{
...
int ginfo(int type)
{
    ...
    case 1:
        return 0; // mouse y
    case 2:
        return 0; // active window id
    case 3:
        return detail::current_buffer; // target window id
    case 4:
        return 0; // window x1
    case 5:
        return 0; // window y1
    case 6:
        return 800; // window x2
    case 7:
        return 600; // window y2
    case 8:
        return 0; // window scroll x
    case 9:
        return 0; // window scroll y
    case 10:
        return 800; // window width
    case 11:
        return 600; // window height
    case 12:
    ...
    case 16:
        return detail::current_tex_buffer().color.r; // current color r
    case 17:
        return detail::current_tex_buffer().color.g; // current color g
    case 18:
        return detail::current_tex_buffer().color.b; // current color b
    case 19:
        return 0; // color mode
    case 20:
        return 0; // resolution x
    case 21:
        return 0; // resolution y
    case 22:
        return detail::current_tex_buffer().x; // current position x
    case 23:
        return detail::current_tex_buffer().y; // current position y
    default:
        assert(0);
    }
}
...
void grotate(int window_id, int src_x, int src_y, double angle, int dst_width, int dst_height)
{
    detail::set_blend_mode();
    snail::detail::enforce_sdl(::SDL_SetTextureAlphaMod(detail::tex_buffers[window_id].texture, detail::current_tex_buffer().color.a));
    ...
    if (window_id == detail::current_buffer)
    {
        ...
        Application::instance().renderer().render_image(
            ...
            src_x,
            src_y,
            detail::current_tex_buffer().width == -1 ? dst_width : detail::current_tex_buffer().width,
            detail::current_tex_buffer().height == -1 ? dst_height : detail::current_tex_buffer().height,
            0,
            0,
            dst_width,
            dst_height);
...
void gsel(int window_id)
{
    ...
    Application::instance().renderer().set_render_target(detail::current_tex_buffer().texture);
}

void gzoom(int dst_width, int dst_height, int window_id, int src_x, int src_y, int src_width, int src_height, int mode)
{
    Application::instance().renderer().set_blend_mode(BlendMode::none);
    snail::detail::enforce_sdl(::SDL_SetTextureAlphaMod(detail::tex_buffers[window_id].texture, 255));
...
void memcpy(elona_vector2<int>& src, int src_i, int src_j, elona_vector2<int>& dst, int dst_i, int dst_j, size_t size)
{
...
void mesbox(std::string& buffer, int width, int height, int style, int max_input_size)
{
    mesbox_detail::message_boxes.emplace_back(std::make_unique<mesbox_detail::MessageBox>(buffer));
}
...
void noteadd(const std::string& text, int index, int overwrite)
{
    ...
    std::for_each(std::begin(lines), std::end(lines), [](const std::string& line) { notemanip::buffer->append(line + '\n'); });
}
...
int rnd(int n)
{
    std::uniform_int_distribution<> dist{0, std::clamp(n-1, 0, (1 << 16) - 1)};
...
{
    auto check_key_pressed = [allow_repeat_keys](int n, Key key, bool is_modifier)
    {
...
void exrand_rnd(int& result, int max)
{
    ...
    {
        result = std::uniform_int_distribution<>{0, max-1}(rnd_detail::random_engine2);
    }
}
...
// FIXME:
// The texture which created with SDL_TEXTUREACCESS_TARGET cannot be locked/unlocked.
namespace gf_detail
{
::SDL_Rect rect;
void* pixels;
...
void map(F f)
{
    // const auto texture = Application::instance().renderer().get_render_target();
    // const auto format = snail::detail::enforce_sdl(
...
{
    // gf_detail::rect = {detail::current_tex_buffer().x, detail::current_tex_buffer().y, width, height};
    // const auto texture = Application::instance().renderer().get_render_target();
    // ::SDL_LockTexture(texture, &gf_detail::rect, &gf_detail::pixels, &gf_detail::pitch);
    // LOG("gfini", gf_detail::pitch, gf_detail::rect.h);
}
...
void memcpy_(std::string& dst, std::string& src, int size, int dst_offset, int src_offset)
{
...
void zOpen(int&, const std::string& filename, int mode, int)
{
    if (mode == 1) // Write
    {
        ...
        gzip_detail::file.open(to_unix_filename(filename), std::ios::out | std::ios::binary);
    }
    else // Read
    {
        LOG("zOpen/r", filename);
        gzip_detail::file.open(to_unix_filename(filename), std::ios::in | std::ios::binary);
    }
    if (!gzip_detail::file)
    {
        LOG("Failed");
...
void zWrite(elona_vector1<int>& data, int, int size)
{
    ...
    for (int i = 0; i < length(data); ++i)
    {
        gzip_detail::file.write(reinterpret_cast<const char*>(&data(i)), sizeof(int));
        size -= sizeof(int);
        if (size == 0)
            return;
    }
...
void zWrite(elona_vector2<int>& data, int, int size)
{
    ...
    for (int j = 0; j < data.j_size(); ++j)
    {
        ...
        {
            gzip_detail::file.write(reinterpret_cast<const char*>(&data(i, j)), sizeof(int));
            size -= sizeof(int);
            if (size == 0)
                return;
        }
...
void zWrite(elona_vector3<int>& data, int, int size)
{
    ...
    for (int k = 0; k < data.k_size(); ++k)
    {
        for (int j = 0; j < data.j_size(); ++j)
        {
            for (int i = 0; i < data.i_size(); ++i)
            {
                gzip_detail::file.write(reinterpret_cast<const char*>(&data(i, j, k)), sizeof(int));
                size -= sizeof(int);
                if (size == 0)
                    return;
            }
...
void zWrite(elona_vector1<std::string>& data, int, int size)
{
    ...
    for (int i = 0; i < length(data); ++i)
    {
        gzip_detail::file.write(reinterpret_cast<const char*>(data(i).c_str()), std::size(data(i)));
...
void zRead(elona_vector2<int>& data, int, int size)
{
    ...
    for (int j = 0; j < data.j_size(); ++j)
    {
        for (int i = 0; i < data.i_size(); ++i)
        {
            data(i, j) = reinterpret_cast<int*>(buf.get())[i + j*data.i_size()];
            size -= sizeof(int);
            if (size == 0)
                return;
        }
...
void zRead(elona_vector2<int>& data, int, int size, int offset)
{
    ...
    for (int j = offset; j < data.j_size(); ++j)
    {
        for (int i = 0; i < data.i_size(); ++i)
        {
            data(i, j) = reinterpret_cast<int*>(buf.get())[i + (j-offset)*data.i_size()];
            size -= sizeof(int);
            if (size == 0)
                return;
        }
...
void zRead(elona_vector3<int>& data, int, int size)
{
    ...
    for (int k = 0; k < data.k_size(); ++k)
    {
        for (int j = 0; j < data.j_size(); ++j)
        {
            for (int i = 0; i < data.i_size(); ++i)
            {
                data(i, j, k) = reinterpret_cast<int*>(buf.get())[i + j*data.i_size() + k*data.i_size()*data.j_size()];
                size -= sizeof(int);
                if (size == 0)
                    return;
            }
